    ...
    () =>
      PROVIDERS.filter(
        (p) =>
          ...
          (!q ||
            `${p.name} ${p.category} ${p.modules.join(" ")}`
              .toLowerCase()
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            External providers powering Canta — payments, FX, tracking, KYC, AI, messaging, CRM and
            more.
          </p>
          <div className="mt-2 inline-flex items-center gap-2 text-[11px] px-2 py-0.5 rounded-full border border-warning/30 bg-warning/10 text-warning">
            <span className="h-1.5 w-1.5 rounded-full bg-warning" /> Demo integrations · not live
            connections
          </div>
        </div>
        <Button onClick={() => toast.success("Integration request submitted")}>
          Request integration
        </Button>
      </div>
      ...
      <Tabs defaultValue="grid" className="space-y-4">
        <TabsList>
          <TabsTrigger value="grid">
            <Plug className="h-3.5 w-3.5 mr-1.5" /> Providers
          </TabsTrigger>
          <TabsTrigger value="webhooks">
            <Activity className="h-3.5 w-3.5 mr-1.5" /> Webhook log
          </TabsTrigger>
        </TabsList>
            ...
            <div className="space-y-1.5 text-xs font-mono">
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="flex justify-between border-b py-1.5">
                  <span>
                    2026-06-15 {String(11 - Math.floor(i / 2)).padStart(2, "0")}:
                  ...
                  <span className={i % 6 === 0 ? "text-destructive" : "text-success"}>
                    {i % 6 === 0 ? "500" : "200"}
                  </span>
                </div>
              ))}
      ...
      <Dialog open={!!config} onOpenChange={(o) => !o && setConfig(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{config?.name} — Configure</DialogTitle>
          </DialogHeader>
          {config && (
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <Switch
                  defaultChecked={config.status === "Connected"}
                  onCheckedChange={(v) => toast.success(v ? "Enabled" : "Disabled")}
                />
              </div>
              <div className="grid grid-cols-2 gap-2 pt-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => toast.success("Connection retried")}
                >
                ...
                >
                  <ScrollText className="h-3.5 w-3.5 mr-1.5" /> View logs
                </Button>
              </div>
            </div>
          )}
  ...
  return (
    <Card className="p-4 shadow-card">
      <div className="flex items-start justify-between gap-2">
        ...
        <Badge variant="outline" className={`text-[10px] gap-1 ${STATUS_TONE[p.status]}`}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => toast.success("Connection retried")}>
          <RefreshCcw className="h-3.5 w-3.5 mr-1.5" /> Retry
        </Button>
...
function chip(active: boolean) {
  return `text-xs px-2.5 py-1.5 rounded-full border ${active ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`;
}
