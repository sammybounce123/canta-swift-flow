...
function SupportPage() {
  ...
  const filtered =
    statusFilter === "All"
      ? tickets
      : tickets.filter((t) => toCustomerStatus(t.status) === statusFilter);
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <LifeBuoy className="h-5 w-5 text-primary" /> My support tickets
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Open a ticket, message the Canta team and track responses.
          </p>
        ...
        <NewTicketDialog
          ...
          customer={ws.name}
          organization={ws.workspaceLabel}
        />
      </div>
      ...
      <Card className="p-3 shadow-card flex flex-wrap gap-2 items-center">
        ...
        {(["All", ...CUSTOMER_STATUSES] as const).map((s) => (
          <button
            key={s}
            onClick={() => setStatusFilter(s as CustomerStatus | "All")}
            className={`text-xs px-2.5 py-1 rounded-full border ${statusFilter === s ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}
          >
            {s}
          </button>
        ))}
                ...
                return (
                  <tr key={t.id} className="border-t hover:bg-secondary/30">
                    ...
                    <td className="py-3 px-3">
                      <Badge variant="outline" className={`text-[10px] ${tone(cs)}`}>
                        {cs}
                      </Badge>
                    </td>
                    ...
                    <td className="py-3 px-3 text-right">
                      <div className="inline-flex flex-wrap gap-1 justify-end">
                        ...
                        <Button
                          ...
                          className="h-7"
                          onClick={() => {
                            if (cs !== "Closed" && cs !== "Resolved")
                              return toast.info("Ticket is already open");
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-sm text-muted-foreground">
                    <MessageCircle className="h-5 w-5 inline mr-1.5" /> No{" "}
                    {ws.workspaceLabel.toLowerCase()} tickets match this filter.
                  </td>
                </tr>
              )}
            </tbody>
...
function TicketDetailDialog({
  ...
  onClose: () => void;
  tone: (s: CustomerStatus) => string | undefined;
}) {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="border-t pt-3">
          ...
          <div className="rounded-lg border border-dashed border-border p-3 text-xs text-muted-foreground">
            No attachments yet. Add files from the reply box when Canta asks for documents or
            receipts.
          </div>
        </div>
        ...
        <div className="border-t pt-3">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            ...
            {ticket.messages
              ...
              .map((m) => (
                <div
                  key={m.id}
                  className={`p-2 rounded-lg text-xs ${m.role === "canta" ? "bg-primary/10" : "bg-secondary/40"}`}
                >
                  <div className="text-[10px] font-semibold text-muted-foreground">
                    {m.role === "canta" ? "Canta Support" : "You"} ·{" "}
          ...
          <div className="mt-2 space-y-2">
            <Textarea
              ...
              placeholder="Write a reply to Canta…"
        ...
        <div className="border-t pt-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="rounded-lg bg-secondary/40 p-2">
              {ticket.createdAt} · Ticket opened for {ticket.linkedRef ?? ticket.issueType}
            </div>
            <div className="rounded-lg bg-secondary/40 p-2">
              {ticket.lastUpdate} · Status is {cs}
            </div>
            {ticket.messages
              .filter((m) => !m.body.startsWith("[note]"))
              .map((m) => (
                <div key={`tl-${m.id}`} className="rounded-lg bg-secondary/40 p-2">
                  ...
                  {m.role === "canta" ? "Canta Support replied" : "Customer message added"}
                </div>
              ))}
          </div>
        </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>
          <Plus className="h-4 w-4 mr-1.5" /> Open new ticket
        </Button>
      </DialogTrigger>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            <Label className="text-xs">
              Linked reference (Trade File / Shipment / Payment / Card)
            </Label>
            <Input
              value={linkedRef}
              onChange={(e) => setLinkedRef(e.target.value)}
              placeholder="TF-… / SHP-… / INV-… / CARD-…"
            />
          </div>
          <div className="col-span-2">
