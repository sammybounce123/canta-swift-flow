...
const templates = [
  {
    i: GraduationCap,
    l: "Tuition Collection",
    d: "Universities & schools",
...
const paymentLinks = [
  ...
  {
    id: "PL-7818",
    customer: "Lagos Med Clinic",
    purpose: "Equipment invoice",
    amount: 4200,
    settleCcy: "EUR",
    status: "Active",
    clicks: 7,
  ...
  {
    id: "PL-7805",
    customer: "Sino Trade Co.",
    purpose: "Bulk order #882",
    amount: 56_000,
    settleCcy: "RMB",
    status: "Active",
    clicks: 21,
  ...
  return (
    <Card className="p-3 sm:p-4 shadow-card min-w-0 overflow-hidden">
      <div className="flex items-start justify-between gap-2">
        ...
        <div
          className={`h-7 w-7 shrink-0 rounded-lg grid place-items-center ${tone ?? "bg-primary/10 text-primary"}`}
        >
          <Icon className="h-3.5 w-3.5" />
        </div>
      </div>
      <div
        className="text-base sm:text-lg lg:text-xl font-semibold mt-2 tabular-nums truncate"
        title={value}
      >
        {value}
      </div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">
          <LinkIcon className="h-4 w-4 mr-1.5" /> New Payment Link
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Customer pays in NGN locally. You receive in your chosen currency.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input placeholder="Jane Adewale" />
          </div>
          <div>
          ...
          <div>
            ...
            <Input placeholder="STU-2241" />
          </div>
          <div className="col-span-2">
            <Label>Payment purpose</Label>
            <Input placeholder={tpl ?? "e.g. Spring 2026 tuition"} />
          </div>
          <div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Sent via email and WhatsApp with a local NGN payment instruction.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
          ...
          <div className="col-span-2">
            <Label>Notes</Label>
            <Textarea placeholder="Description of goods or services" />
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={() => setOpen(false)}>
          ...
          <Button
            onClick={() => {
              setOpen(false);
              toast.success("Invoice created & sent to payer");
            }}
          >
            Send invoice
          </Button>
...
function Collections() {
  ...
  const isMerchant =
    typeof window !== "undefined" ? localStorage.getItem("canta:mode") === "Global Merchant" : true;
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Settlement timing depends on payment method, currency, compliance review, and payout rail."
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        <KPI
          label="Top Payer"
          value={topPayers[0].name.split(" ")[0]}
          sub={fmtMoney(topPayers[0].total, topPayers[0].ccy)}
          icon={TrendingUp}
        />
        <KPI
          label="Reconciliation Issues"
          value={String(totals.recon)}
          sub="Unmatched / underpaid"
          icon={AlertTriangle}
          tone="bg-warning/10 text-warning"
        />
      </div>
      ...
      {isMerchant && (
        <Card className="p-5 shadow-card">
          <div className="text-sm font-semibold mb-3 flex items-center gap-2">
            <Globe className="h-4 w-4" /> Start with a use-case template
          </div>
      ...
      <Tabs defaultValue="links" className="space-y-4">
        <TabsList
          aria-label="Global Collections sections"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs text-muted-foreground bg-secondary/40">
                    ...
                    <th className="px-4 py-3">Settle in</th>
                    ...
                    .map((p) => (
                      <tr key={p.id} className="border-t border-border">
                        ...
                        <td className="px-4 py-3 text-right tabular-nums font-semibold">
                          {fmtMoney(p.amount, p.settleCcy)}
                        </td>
                        <td className="px-4 py-3 text-muted-foreground">{p.due}</td>
                        <td className="px-4 py-3">
                          <Badge
                            variant="outline"
                            className={`text-[10px] ${statusTone(p.status)}`}
                          >
                            {p.status}
                          </Badge>
                        </td>
                ...
                <tbody>
                  {invoices.map((i) => (
                    <tr key={i.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3">
                        <Badge variant="outline" className={`text-[10px] ${statusTone(i.status)}`}>
                          {i.status}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => toast.success(`${i.id} resent`)}
                        >
                ...
                <tbody>
                  {payers.map((p) => (
                    <tr key={p.name} className="border-t border-border">
                      ...
                      <td className="px-4 py-3">
                        <Badge variant="outline" className={`text-[10px] ${statusTone(p.status)}`}>
                          {p.status}
                        </Badge>
                      </td>
                    </tr>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs text-muted-foreground bg-secondary/40">
                    ...
                    <th className="px-4 py-3 text-right">Collected (NGN)</th>
                    <th className="px-4 py-3 text-right">Fees</th>
                    <th className="px-4 py-3">FX Rate</th>
                    <th className="px-4 py-3">Settle Ccy</th>
                    <th className="px-4 py-3 text-right">Settle Amount</th>
                ...
                <tbody>
                  {settlements.map((s) => (
                    <tr key={s.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums font-semibold">
                        {fmtMoney(s.settleAmt, s.settleCcy === "RMB" ? "CNY" : s.settleCcy)}
                      </td>
          ...
          <Card className="shadow-card overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => toast.success("Auto-match run complete")}
              >
                ...
                <tbody>
                  {reconciliation.map((r) => (
                    <tr key={r.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums">
                        {r.expected ? fmtMoney(r.expected, "USD") : "—"}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums font-semibold">
                        {fmtMoney(r.received, "USD")}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {r.status === "Matched" ? (
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => toast.success("Investigation opened")}
                          >
            ...
            {[
              ...
              <Card key={r.l} className="p-4 shadow-card flex items-start justify-between gap-3">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => toast.success(`${r.l} downloading`)}
                >
      ...
      <Card className="p-5 shadow-card">
        ...
        <div className="text-xs text-muted-foreground mb-4">
          Where merchant staff spend goes this month · USD
        </div>
            ...
            return (
              <div key={d.dept}>
                ...
                <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                  <div
                    className={`h-full ${pct > 85 ? "bg-destructive" : pct > 60 ? "bg-warning" : "bg-success"}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Icon className="h-4 w-4 text-primary" /> {template.l} — guided flow
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-3">
          {step === 1 && (
            <>
              ...
              <Input
                ...
                placeholder={isTuition ? "e.g. Adaeze Okafor" : "e.g. Adaeze Okafor"}
              />
          ...
          {step === 2 && (
            <>
              <Label>
                {isTuition ? "Tuition invoice / payment reference" : "Invoice / reference"}
              </Label>
              <Input
                value={reference}
                onChange={(e) => setReference(e.target.value)}
                placeholder={`${template.purpose} ref`}
              />
              <Label>Amount</Label>
              <Input
                value={amount}
          ...
          {step === 4 && (
            <>
              ...
              <div className="text-xs text-muted-foreground">
                Funds collected in NGN, settled to your {ccy} wallet at the live FX rate.
              </div>
            </>
          )}
          {step === 5 && (
            <div className="p-4 rounded-lg bg-secondary/40 border border-border text-sm">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                A matching report is generated automatically when NGN funds arrive — matched against{" "}
                {reference || "AUTO-REF"}.
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-muted-foreground">Payer:</span> {payer || "—"}
                </div>
                <div>
                ...
                <div>
                  <span className="text-muted-foreground">Amount:</span> {amount || "0.00"} {ccy}
                </div>
                <div>
                  <span className="text-muted-foreground">Settle to:</span> {ccy} wallet
                </div>
              </div>
            </div>
          )}
          {step === 6 && (
            <div className="p-4 rounded-lg bg-success/10 border border-success/30 text-sm">
              <div className="font-semibold text-success flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4" /> Settlement status
              </div>
              <div className="mt-2 text-xs">
                Once payer funds arrive, settlement to your {ccy} wallet is scheduled{" "}
                <strong>T+1</strong>. You and the payer both receive a confirmation receipt.
              </div>
            </div>
          )}
        </div>
