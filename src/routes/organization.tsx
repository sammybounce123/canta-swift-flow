...
const seedUsers = [
  ...
  },
  {
    id: "U7",
    name: "James Okafor",
    email: "james@nigerdelta.ng",
    role: "Operations Staff",
    branch: "Lagos HQ",
    dept: "Sales",
    status: "Pending" as const,
    lastSeen: "Never (invited)",
  },
  {
    id: "U8",
    name: "Ngozi Okeke",
    email: "ngozi@nigerdelta.ng",
    role: "Accountant",
...
const costCenters = [
  ...
  {
    id: "CC-300",
    name: "Digital Marketing — Brand Q2",
    owner: "Marketing",
    budget: 80_000,
    used: 51_200,
  },
...
const ROLE_GROUPS = [
  ...
  },
  {
    ...
    roles: [
      "Canta Super Admin",
      "Canta Trade Officer",
      "Canta Compliance Officer",
      "Canta Treasury Officer",
      "Canta Sales Agent",
      "Canta Support Agent",
      "Canta Collections Officer",
      "Canta Freight Operations",
      "Canta Auditor",
    ],
  },
];
...
const PERMISSIONS = [
  ...
  },
  {
    ...
    items: [
      "view wallets",
      "approve FX conversions",
      "manage beneficiaries",
      "create payment links",
      "approve collections settlement",
    ],
  },
  {
    group: "Enterprise",
  ...
  },
  {
    ...
    items: [
      "create buyer record",
      "view buyer verification status",
      "create invoice",
  ...
  },
  {
    ...
    items: [
      "view all customers",
      "create trade file for customer",
      "edit trade file",
      "assign customer to sales agent",
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-3xl">
            Company → Branches → Departments → Teams → Users → Roles & Permissions. Wallets, trade
            files, approvals and collections all attach to this hierarchy.
          </p>
        </div>
        <div className="flex gap-2">
          <Link to="/team">
            <Button variant="outline">
              Team & Roles <ArrowRight className="h-3.5 w-3.5 ml-1.5" />
            </Button>
          </Link>
          <Link to="/approvals">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Stat
          icon={MapPin}
          label="Branches"
          value={String(seedBranches.length)}
          sub={`${seedBranches.reduce((s, b) => s + b.staff, 0)} staff`}
        />
        <Stat
          icon={Layers}
          label="Departments"
          value={String(seedDepts.length)}
        ...
        <Stat
          icon={Shield}
          label="Users"
          value={String(seedUsers.length)}
          sub={`${seedUsers.filter((u) => u.status === "Active").length} active`}
        />
      </div>
        ...
        <TabsContent value="profile" className="mt-4">
          <Card className="p-6 shadow-card">
            ...
            <div className="mt-4 flex gap-2">
              ...
              <Button variant="ghost" onClick={() => toast.success("Logo upload opened")}>
                Upload logo
              </Button>
            </div>
          </Card>
            ...
            {ROLE_GROUPS.map((g) => (
              <Card key={g.label} className="p-5 shadow-card">
                ...
                <Button
                  ...
                  size="sm"
                  className="mt-4"
                  onClick={() => toast.success(`${g.label} template duplicated`)}
                >
        ...
        <TabsContent value="cost" className="mt-4">
          <Card className="p-5 shadow-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button size="sm" onClick={() => toast.success("Cost center created")}>
                <Plus className="h-3.5 w-3.5 mr-1.5" /> New cost center
              </Button>
            </div>
          ...
          <Card className="p-5 shadow-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                {
                  name: "Trade file creation",
                  threshold: "Always",
                  chain: "Procurement → Logistics",
                },
                {
                  name: "Collections settlement",
                  threshold: "USD 100,000",
                  chain: "Reconciliation → Finance",
                },
              ].map((w) => (
                <div
                  key={w.name}
...
function UsersPanel() {
  ...
  const toggleStatus = (id: string) => {
    setUsers((u) =>
      u.map((x) =>
        x.id === id ? { ...x, status: x.status === "Active" ? "Deactivated" : "Active" } : x,
      ),
    );
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-1 justify-end">
                    <Button
                      ...
                      onClick={() => toast.success("Activity log opened")}
                    >
                      Activity
                    </Button>
                    <Button
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-7 w-7 p-0"
                      onClick={() => toast.success("User removed")}
                    >
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="bg-primary">
          <UserPlus className="h-3.5 w-3.5 mr-1.5" /> Invite sub-user
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            They'll get a magic-link email and a WhatsApp prompt.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <Label>Full name</Label>
            <Input placeholder="e.g. James Okafor" />
          </div>
          <div className="col-span-2">
            <Label>Work email</Label>
            <Input type="email" placeholder="james@company.com" />
          </div>
          <div>
            <Label>Branch</Label>
          ...
          <div className="col-span-2 p-3 rounded-lg bg-secondary/40 border border-border text-xs">
            <div className="flex items-center gap-1.5 font-medium mb-1">
              <CheckCircle2 className="h-3.5 w-3.5 text-success" /> Approval required for high-value
              payments by default.
            </div>
            <div className="text-muted-foreground">
              You can fine-tune permissions after the user accepts.
            </div>
          </div>
        </div>
        <DialogFooter>
  ...
  return (
    <Card className="p-5 shadow-card overflow-x-auto">
      ...
      <div className="text-xs text-muted-foreground mb-4">
        Toggle defaults per role. Templates are a starting point — admins can override at the user
        level.
      </div>
            ...
            <>
              <tr key={`h-${g.group}`} className="bg-secondary/20">
                <td
                  ...
                  className="px-3 py-1.5 text-[10px] uppercase tracking-widest text-muted-foreground"
                >
                  {g.group}
                </td>
              </tr>
