...
import {
  ...
  AlertTriangle, PiggyBank, ArrowRight, Plus, Home, ShieldCheck, Sparkles, Award,
} from "lucide-react";
import {
...
import {
  CASES, SOLICITORS, formatGBP, statusTone, getSolicitor, getMarketer,
  LEADS, visibleCases, visibleLeads, marketerPerformance, PARTNER_ROLES,
} from "@/lib/partner";
...
function PartnerDashboard() {
  ...
  if (role === "marketer") return <MarketerDashboard userId={userId} name={user?.name ?? "Marketer"} />;
...
function AdminDashboard() {
  ...
  const activeLeads = LEADS.filter((l) => !["Lost", "Not Ready", "Converted to Payment Case"].includes(l.status)).length;
  ...
  const kpis = [
    { l: "Clients referred (all marketers)", v: totalReferred.toString(), icon: Users, tone: "text-primary" },
    ...
    { l: "Est. partner commission", v: formatGBP(Math.round(totalGBP * 0.005)), icon: PiggyBank, tone: "text-primary" },
  ];

  const monthly = [
    { m: "Jan", payouts: 820_000 }, { m: "Feb", payouts: 1_120_000 },
    { m: "Mar", payouts: 980_000 }, { m: "Apr", payouts: 1_540_000 },
    { m: "May", payouts: 1_810_000 }, { m: "Jun", payouts: 1_245_000 },
  ];

  const perf = marketerPerformance().sort((a, b) => b.totalPaidGBP - a.totalPaidGBP);
  const topMarketers = perf.slice(0, 5);
  const colors = ["oklch(0.36 0.12 260)", "oklch(0.78 0.16 175)", "oklch(0.6 0.18 240)", "oklch(0.72 0.17 160)", "oklch(0.65 0.18 30)"];
  const conversionData = perf.map((p, i) => ({ name: p.marketer.name.split(" ")[0], value: Math.round(p.conversionRate * 100), fill: colors[i % colors.length] }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <Card className="lg:col-span-2 p-6 shadow-card">
          ...
          <div className="h-56 mt-4">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={conversionData} dataKey="value" innerRadius={50} outerRadius={85} paddingAngle={2}>
                  {conversionData.map((d, i) => <Cell key={i} fill={d.fill} />)}
                </Pie>
      ...
      <Card className="p-6 shadow-card">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-sm font-semibold flex items-center gap-2"><Award className="h-4 w-4 text-primary" /> Top marketers</div>
            <div className="text-xs text-muted-foreground">Ranked by GBP paid to solicitors</div>
          </div>
          <Button asChild variant="ghost" size="sm"><Link to="/partner/marketers">All marketers <ArrowRight className="h-3.5 w-3.5 ml-1" /></Link></Button>
        </div>
            ...
            <tbody>
              {topMarketers.map((p) => (
                <tr key={p.marketer.id} className="border-b last:border-0">
                  ...
                  <td className="py-3 px-2 text-right tabular-nums font-medium">{formatGBP(p.totalPaidGBP)}</td>
                  <td className="py-3 px-2 text-right tabular-nums">{Math.round(p.conversionRate * 100)}%</td>
                  <td className="py-3 px-2 text-xs text-muted-foreground tabular-nums">{p.lastActivity}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card className="p-6 shadow-card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button asChild variant="ghost" size="sm"><Link to="/partner/cases">All cases <ArrowRight className="h-3.5 w-3.5 ml-1" /></Link></Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-3">
          {SOLICITORS.map((s) => (
            <div key={s.id} className="rounded-lg border bg-background p-3">
              ...
              <div className="text-sm font-semibold tabular-nums mt-2">{formatGBP(s.totalPayouts)}</div>
            </div>
          ))}
        </div>
      </Card>
...
function MarketerDashboard({ userId, name }: { userId: string; name: string }) {
  ...
  const successful = myCases.filter((c) => ["Paid to Solicitor", "Receipt Uploaded"].includes(c.status));
  const active = myCases.filter((c) => !["Paid to Solicitor", "Receipt Uploaded", "Failed / Returned", "Cancelled"].includes(c.status));
  const paid = successful.reduce((s, c) => s + c.amountGBP, 0);
  const activeLeads = myLeads.filter((l) => !["Lost", "Not Ready", "Converted to Payment Case"].includes(l.status));
  const needAction = myCases.filter((c) => ["Awaiting Client Funding", "KYC Pending", "Documents Requested"].includes(c.status as string));
  const pendingDocs = myCases.filter((c) => c.status === "KYC Pending" || c.status === "Awaiting Client Funding").length;
  ...
  const kpis = [
    ...
    { l: "My active payment cases", v: active.length.toString(), icon: FileText, tone: "text-primary" },
    { l: "My successful payouts", v: successful.length.toString(), icon: CheckCircle2, tone: "text-success" },
    { l: "My total GBP paid out", v: formatGBP(paid), icon: Banknote, tone: "text-success" },
    { l: "Active leads in pipeline", v: activeLeads.length.toString(), icon: ArrowLeftRight, tone: "text-warning" },
    { l: "Pending documents", v: pendingDocs.toString(), icon: Clock, tone: "text-warning" },
    { l: "Clients needing action", v: needAction.length.toString(), icon: AlertTriangle, tone: "text-destructive" },
    { l: "My conversion rate", v: `${conv}%`, icon: PiggyBank, tone: "text-primary" },
  ];

  return (
    <div className="space-y-6">
      <Hero title={`Welcome back, ${name.split(" ")[0]}`} subtitle="Your referrals, leads and successful solicitor payouts." />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map((k) => <Kpi key={k.l} {...k} />)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card className="p-6 shadow-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button asChild size="sm" variant="ghost"><Link to="/partner/leads">View all <ArrowRight className="h-3.5 w-3.5 ml-1" /></Link></Button>
          </div>
            ...
            {activeLeads.slice(0, 6).map((l) => (
              <li key={l.id} className="py-2.5 flex items-center justify-between gap-3">
                ...
                <div className="text-right">
                  <div className="text-xs font-medium tabular-nums">{formatGBP(l.expectedAmountGBP)}</div>
                  <Badge variant="outline" className="text-[10px] mt-1">{l.status}</Badge>
                </div>
              </li>
            ))}
            {activeLeads.length === 0 && <li className="py-6 text-center text-sm text-muted-foreground">No active leads.</li>}
          </ul>
        </Card>
        ...
        <Card className="p-6 shadow-card">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-xs text-muted-foreground">Awaiting funding, KYC or documents</div>
            </div>
            <Button asChild size="sm" variant="ghost"><Link to="/partner/cases">View all <ArrowRight className="h-3.5 w-3.5 ml-1" /></Link></Button>
          </div>
          <ul className="text-sm divide-y">
            {needAction.slice(0, 6).map((c) => (
              <li key={c.id} className="py-2.5 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <Link to="/partner/cases/$caseId" params={{ caseId: c.id }} className="font-medium hover:underline">{c.clientName}</Link>
            ...
            {needAction.length === 0 && <li className="py-6 text-center text-sm text-muted-foreground">No actions required.</li>}
          </ul>
        </Card>
      </div>
      ...
      <Card className="p-6 shadow-card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button asChild size="sm" variant="ghost"><Link to="/partner/payouts">All payouts <ArrowRight className="h-3.5 w-3.5 ml-1" /></Link></Button>
        </div>
    ...
    <Card className="p-6 shadow-card bg-gradient-to-br from-primary/5 via-card to-accent/5 border-primary/20">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline"><Link to="/partner/leads">Leads</Link></Button>
...
function Kpi({ l, v, icon: Icon, tone }: { l: string; v: string; icon: React.ComponentType<{ className?: string }>; tone: string }) {
  return (
          ...
          {rows.map((c) => {
            ...
            return (
              <tr key={c.id} className="border-b last:border-0 hover:bg-secondary/40">
                ...
                <td className="py-3 px-2"><Badge variant="outline" className={`text-[10px] ${statusTone(c.status)}`}>{c.status}</Badge></td>
