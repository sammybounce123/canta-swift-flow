...
function CommissionsPage() {
  ...
  const canSeeAll =
    role === "partner_admin" || role === "finance_viewer" || role === "partner_manager";
  ...
  const exportCSV = () => {
    const header =
      "ID,Case,Client,Marketer,Payout (GBP),Rate %,Estimated,Approved,Paid,Status,Payment Date\n";
    const rows = list
      .map((c) =>
        [
          c.id,
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <Percent className="h-5 w-5 text-primary" /> Commissions
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {isMarketer
              ? "Your attributed commission across closed cases."
              : "Partner & marketer commissions across all Kingsbridge Property Partners cases."}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat
          label="Paid"
          value={formatGBP(
            list.filter((c) => c.status === "Paid").reduce((s, c) => s + (c.paid ?? 0), 0),
          )}
        />
        <Stat
          label="Approved"
          value={formatGBP(
            list.filter((c) => c.status === "Approved").reduce((s, c) => s + (c.approved ?? 0), 0),
          )}
        />
        <Stat
                  ...
                  <td className="py-3 px-3 text-right">
                    {canSeeAll && (
                      <Select
                        value={c.status}
                        onValueChange={(v) => {
                          updateCommissionStatus(c.id, v as CommissionStatus);
              ...
              {list.length === 0 && (
                <tr>
                  <td colSpan={9} className="py-10 text-center text-sm text-muted-foreground">
                    <CheckCircle2 className="h-5 w-5 inline mr-1.5" /> No commissions yet.
                  </td>
                </tr>
              )}
            </tbody>
