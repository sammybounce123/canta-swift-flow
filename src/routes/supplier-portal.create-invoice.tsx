...
export const Route = createFileRoute("/supplier-portal/create-invoice")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Quote NGN to RMB, generate an invoice and send it to your Nigerian buyer.",
      },
    ],
  }),
...
function CreateInvoicePage() {
  ...
  const generateQuote = () => {
    if (rmb <= 0) {
      toast.error("Enter the RMB amount you want to receive.");
      return;
    }
    if (!goods.trim()) {
      toast.error("Add a goods / order description.");
      return;
    }
  ...
  const generateInvoice = () => {
    ...
    if (Date.now() > quote.expiresAt) {
      toast.error("Quote expired. Generate a new rate before creating the invoice.");
      return;
    }
    ...
    if (!buyer.whatsapp.trim() && !buyer.email.trim()) {
      toast.error("Add a buyer WhatsApp number or email.");
      return;
    }
    ...
    <div className="space-y-4">
      <Card className="p-4">
        <div className="flex flex-wrap gap-2 text-xs">
          {[
            ...
            <Badge key={n} variant={Number(n) === step ? "default" : "outline"}>
              {n}. {label}
            </Badge>
          ))}
        </div>
      ...
      {step === 1 && (
        <Card className="space-y-3 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                placeholder="Bluetooth speakers x 500"
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" disabled={!quote} onClick={() => setStep(2)}>
              Generate Invoice
            </Button>
          </div>
        </Card>
      ...
      {step === 2 && quote && (
        <Card className="space-y-3 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <Label className="text-xs">Buyer company / name</Label>
              <Input
                className="mt-1"
                value={buyer.company}
                onChange={(e) => setBuyer({ ...buyer, company: e.target.value })}
                placeholder="Zenith Imports Nigeria"
              />
            </div>
            <div>
              <Label className="text-xs">Buyer WhatsApp number</Label>
              <Input
                className="mt-1"
            ...
            <div>
              ...
              <Input
                ...
                placeholder="buyer@example.ng"
              />
            </div>
            <div>
              <Label className="text-xs">WeChat ID (optional)</Label>
              <Input
                className="mt-1"
                value={buyer.wechat}
                onChange={(e) => setBuyer({ ...buyer, wechat: e.target.value })}
              />
            </div>
            <div>
              <Label className="text-xs">Goods / order description</Label>
              <Input className="mt-1" value={goods} onChange={(e) => setGoods(e.target.value)} />
            </div>
            <div>
              ...
              <Input
                ...
                type="date"
                value={buyer.dueDate}
                onChange={(e) => setBuyer({ ...buyer, dueDate: e.target.value })}
              />
            </div>
            <div className="sm:col-span-2">
              ...
              <Textarea
                ...
                placeholder="50% deposit, balance before shipment"
              />
            </div>
          </div>
          <div className="grid gap-2 rounded-lg border bg-muted/30 p-3 text-sm sm:grid-cols-4">
      ...
      {step === 3 && invoice && (
        <Card className="space-y-3 p-4">
          ...
          <div className="grid gap-2 rounded-lg border p-3 text-sm sm:grid-cols-3">
            ...
            <Kv
              k="NGN collection account"
              v={`${NGN_COLLECTION_ACCOUNT.bankName} · ${NGN_COLLECTION_ACCOUNT.accountNumber}`}
            />
          </div>

          {isInvoiceQuoteExpired(invoice) ? (
            <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-xs text-destructive">
              Quote expired before payment. Generate a new rate before conversion — expired quotes
              cannot be sent to buyers.
            </div>
          ) : (
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  window.open(
                    `https://wa.me/${invoice.buyerWhatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(wechatMessage(invoice))}`,
                    "_blank",
                    "noopener",
                  );
                  toast.success(
                    "Opening WhatsApp — review and send the message yourself. Canta does not send it for you.",
                  );
                }}
              >
                <MessageCircle className="mr-2 h-4 w-4" /> Send by WhatsApp
              </Button>
              <Button
                size="sm"
                variant="outline"
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  toast.success("WeChat message copied — paste it in WeChat");
                }}
              >
                <Copy className="mr-2 h-4 w-4" /> Copy WeChat message
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => {
                  copyText(invoice.paymentLink);
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => navigate({ to: "/supplier-portal/invoices" })}
            >
