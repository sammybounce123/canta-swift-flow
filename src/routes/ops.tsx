...
import { PartnerComplianceQueue } from "@/components/ops/PartnerComplianceQueue";
  ...
  head: () => ({
    meta: [
      { title: "Ops Console — Canta" },
      {
        name: "description",
        content:
          "Internal operations overview across verification, payments, support, WhatsApp, and compliance.",
      },
...
const METRICS: Metric[] = [
  {
    label: "Pending KYB / verification reviews",
    value: "14",
    hint: "Awaiting compliance action",
    tone: "warn",
    icon: ShieldCheck,
    to: "/verification-center",
  },
  {
    label: "Recent payment requests (24h)",
    value: "38",
  ...
  },
  {
    ...
    value: "9",
    hint: "Cross-workspace queue",
    tone: "default",
    icon: LifeBuoy,
    to: "/support",
  },
  {
    label: "WhatsApp shipment / BL checks",
    value: "22",
  ...
  {
    label: "Failed or flagged payments",
    value: "3",
    hint: "Requires operator triage",
    tone: "danger",
    icon: AlertTriangle,
    to: "/treasury",
  },
...
const RECENT_ACTIVITY = [
  {
    at: "2m ago",
    kind: "Verification",
    text: "Kano Distributors submitted updated CAC docs",
    to: "/verification-center",
  },
  {
    at: "14m ago",
    kind: "Payment",
    text: "PR-3080 flagged for compliance review",
    to: "/compliance",
  },
  {
    at: "38m ago",
    kind: "Support",
    text: "Ticket #4821 escalated — Enterprise Treasury",
    to: "/support",
  },
  {
    at: "1h ago",
    kind: "WhatsApp",
    text: "BL verification requested for TF-2026-0214",
    to: "/whatsapp",
  },
  {
    at: "3h ago",
    kind: "Payment",
    text: "PR-3055 failed at settlement rail — retry queued",
    to: "/treasury",
  },
];

const TRADE_FILE_MONITOR: {
  id: string;
  supplier: string;
  importer: string;
  amount: string;
  stage: string;
  type: "External supplier" | "Canta supplier";
}[] = [
  {
    id: "TF-2026-0214",
    supplier: "Shenzhen Hua Tech Co.",
  ...
  },
  {
    ...
    amount: "GBP 92,400",
    stage: "Settlement in progress",
    type: "External supplier",
  },
  {
    id: "TF-2026-0203",
    supplier: "Contoso Industries",
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Internal operations overview. Monitor verification, payments, support, WhatsApp shipment
            checks, and compliance activity across all workspaces. Counts and activity below are
            illustrative for this demo.
          </p>
        </div>
        <Button asChild variant="outline" size="sm">
          <Link to="/reports">
            <BarChart3 className="h-4 w-4 mr-1.5" /> Open reports
          </Link>
        </Button>
      </div>
      ...
      <Card className="p-4">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Importers can pay suppliers that have no Canta account. Each Trade File shows whether the
          beneficiary is an external supplier or a Supplier Portal user.
        </p>
        <ul className="divide-y divide-border">
          {TRADE_FILE_MONITOR.map((t) => (
            <li key={t.id} className="py-2.5 flex items-center justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {t.importer} · {t.amount} · {t.stage}
                </div>
              </div>
              <Badge
                variant={t.type === "Canta supplier" ? "secondary" : "outline"}
                className="text-[10px]"
              >
                {t.type}
              </Badge>
      ...
      <p className="text-xs text-muted-foreground">
        Ops Console is an internal Canta view. Supplier RMB Settlement balances and importer Trade
        File details are only referenced here at an operational level and remain owned by their
