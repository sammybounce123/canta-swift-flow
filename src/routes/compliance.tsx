...
function CompliancePack() {
  ...
  const overallStatus: DocState = Object.values(MY_DOCS).some((s) => s === "Missing")
    ? "More Info Required"
    : Object.values(MY_DOCS).some((s) => s === "Pending Review")
      ? "Pending Review"
      : Object.values(MY_DOCS).some((s) => s === "More Info Required")
        ? "More Info Required"
        : "Verified";
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-primary" /> Compliance Pack
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Your account verification, documents, beneficiaries and supplier checks — all in one
            place.
          </p>
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => toast.success("Compliance Pack PDF queued")}
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <Badge variant="outline" className={`text-xs ${docTone(overallStatus)}`}>
                {overallStatus === "Verified" ? "Approved" : overallStatus}
              </Badge>
              <span className="text-sm text-muted-foreground">
                {verifiedCount} of {REQUIRED_DOCS.length} required items verified
              </span>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="h-8 w-8 rounded-lg bg-secondary grid place-items-center">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className={`text-[10px] ${docTone(state)}`}>
                    <Icon className="h-3 w-3 mr-1 inline" /> {state}
                  </Badge>
                  {state !== "Verified" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => toast.success(`Upload dialog opened for ${d.label}`)}
                    >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                Settlement batches, treasury sweeps and payout processing are managed from the Ops
                Console, not here.
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-5 shadow-card">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="text-sm font-semibold">Beneficiary verification</div>
          </div>
          <ul className="space-y-2 text-sm">
            <li className="flex justify-between">
              <span>Quinn Solicitors LLP</span>
              <Badge variant="outline" className={`text-[10px] ${docTone("Verified")}`}>
                Verified
              </Badge>
            </li>
            <li className="flex justify-between">
              <span>Shenzhen LedTech</span>
              <Badge variant="outline" className={`text-[10px] ${docTone("Pending Review")}`}>
                Pending Review
              </Badge>
            </li>
          </ul>
        ...
        <Card className="p-5 shadow-card">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="text-sm font-semibold">Supplier verification</div>
          </div>
          <ul className="space-y-2 text-sm">
            <li className="flex justify-between">
              <span>Yiwu PolyPack Trading</span>
              <Badge variant="outline" className={`text-[10px] ${docTone("More Info Required")}`}>
                More Info Required
              </Badge>
            </li>
            <li className="flex justify-between">
              <span>Dubai Auto Parts Hub</span>
              <Badge variant="outline" className={`text-[10px] ${docTone("Verified")}`}>
                Verified
              </Badge>
            </li>
