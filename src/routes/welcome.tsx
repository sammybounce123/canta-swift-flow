    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="mx-auto grid h-16 max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 sm:px-6">
          <Link to="/" className="flex min-w-0 items-center gap-2">
            <div className="h-7 w-7 shrink-0 rounded-md bg-primary grid place-items-center text-primary-foreground text-sm font-bold">C</div>
          ...
          <div className="shrink-0 text-xs text-muted-foreground">
            ...
            <Link to="/dashboard" className="text-primary font-medium">Continue to app</Link>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="mb-8 text-center sm:mb-10">
          ...
          <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl md:text-4xl">Choose your workspace</h1>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className={`h-11 w-11 shrink-0 rounded-2xl grid place-items-center sm:h-12 sm:w-12 ${TONES[s.id]}`}>
                    <Icon className="h-6 w-6" />
                  </div>
                  {hot && <CheckCircle2 className="h-4 w-4 shrink-0 text-accent" />}
                </div>

                <div className="mt-4 text-base font-semibold tracking-tight sm:mt-5 sm:text-lg">{s.label}</div>
                ...
                <div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-5">
                  <Badge variant="outline" className="max-w-full truncate text-[10px]">Routes to {s.shortLabel}</Badge>
