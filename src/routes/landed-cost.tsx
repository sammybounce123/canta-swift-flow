...
function LandedCostPage() {
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Landed cost is an estimate based on the values you enter."
      />
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3 sm:flex sm:flex-wrap sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <h1 className="flex items-center gap-2 text-xl font-semibold tracking-tight sm:text-2xl">
            ...
            <span className="truncate">Landed Cost</span>
          </h1>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            Know your true cost per unit before goods arrive — and see your projected profit.
          </p>
        </div>
        <Button className="shrink-0" onClick={() => toast.success("Estimate saved")}>
          Save estimate
        </Button>
      </header>
      ...
      <Card className="p-4 shadow-card">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div>
            <Label>Shipment / Trade file</Label>
            <Input
              value={shipment}
        ...
        <Card className="min-w-0 p-4 shadow-card">
          ...
          <div className="space-y-2">
            {lines.map((l) => (
              <div
                key={l.id}
                className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 sm:grid-cols-[minmax(0,1fr)_140px_auto]"
              >
                <Input
        ...
        <Card className="p-4 shadow-card h-fit space-y-3">
          ...
          <div className="border-t pt-2">
            <Row
              ...
              value={fmtMoney(totals.profit, ccy)}
