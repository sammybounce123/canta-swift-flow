  ...
  supplier_dashboard: [
    {
      ...
      ts: "2026-06-23 10:15",
      module: "Invoices",
      action: "Invoice issued to buyer",
      target: "INV-2030",
      status: "Success",
    },
    {
      ...
      ts: "2026-06-23 11:02",
      module: "Escrow",
      action: "Escrow release requested",
      target: "ESC-7711",
      status: "Pending",
    },
  ],
  enterprise_treasury: [
    {
      ...
      ts: "2026-06-23 09:14",
      module: "FX",
      action: "FX quote generated",
      target: "USD→NGN @ 1,612.40",
      status: "Success",
    },
    {
      id: "A-5002",
...
const DEMO_AUDIT_SEED: AuditEntry[] = [
  {
    ...
    workspace: "enterprise_treasury",
    action: "Flagged transaction as high-risk",
    entity: "TX-8842",
    result: "Success",
    detail: "Demo seed — unusual settlement pattern",
  },
  {
    id: "AUD-DEMO02",
    ts: "2026-06-22 14:20",
    actor: "Ada N. (Ops)",
    workspace: "partner_property",
    action: "Dispute status changed to Resolved",
    entity: "DSP-001",
    result: "Success",
    detail: "Demo seed",
  },
  {
    ...
    ts: "2026-06-21 11:47",
    actor: "Trust & Safety",
    workspace: "importer_portal",
    action: "Buyer verification reviewed",
    entity: "ALL-002",
    result: "Pending",
    detail: "Demo seed — awaiting more documents",
  },
];
    ...
    () =>
      auditEntries.filter(
        (e) =>
          ...
          (aq === "" ||
            `${e.actor} ${e.workspace} ${e.action} ${e.entity}`
              .toLowerCase()
              .includes(aq.toLowerCase())),
      ),
  ...
  function exportCsv() {
    ...
    if (tab === "audit-trail") {
      ...
      filteredAudit.forEach((e) =>
        rows.push([e.ts, e.actor, e.workspace, e.action, e.entity, e.result]),
      );
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your own account activity across this workspace.
          </p>
          <div className="text-[11px] text-muted-foreground mt-1">
            Signed in as <span className="font-semibold text-foreground">{ws.name}</span> ·{" "}
            {ws.title} ·{" "}
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
        <TabsList>
          <TabsTrigger value="my-activity">
            <History className="h-3.5 w-3.5 mr-1" /> My activity
          </TabsTrigger>
          <TabsTrigger value="audit-trail">
            <ListChecks className="h-3.5 w-3.5 mr-1" /> Audit trail
          </TabsTrigger>
        </TabsList>
                ...
                {filtered.map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-[10px] ${e.status === "Success" ? "bg-success/15 text-success border-success/30" : "bg-warning/15 text-warning border-warning/30"}`}
                      >
                        {e.status}
                      </Badge>
                    </TableCell>
        ...
        <TabsContent value="audit-trail" className="space-y-4 mt-4">
          <p className="text-xs text-muted-foreground max-w-2xl">
            System-wide audit trail sourced from compliance and treasury actions (flags, disputes,
            verifications), plus seeded demo entries — click a row for details.
          </p>
          <Card className="p-3 shadow-card flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <Input
                  className="pl-9"
                  placeholder="Search actor, workspace, action or entity…"
                  value={aq}
                ...
                {filteredAudit.map((e) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-[10px] ${e.result === "Success" ? "bg-success/15 text-success border-success/30" : e.result === "Pending" ? "bg-warning/15 text-warning border-warning/30" : "bg-destructive/10 text-destructive border-destructive/30"}`}
                      >
                        {e.result}
                      </Badge>
                    </TableCell>
            ...
            <>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between border-b border-border py-1.5">
                  ...
                  <span className="font-medium">{detail.ts}</span>
                </div>
                <div className="flex justify-between border-b border-border py-1.5">
                ...
                <div className="flex justify-between border-b border-border py-1.5">
                  ...
                  <span className="font-medium">{detail.workspace}</span>
                </div>
                <div className="flex justify-between border-b border-border py-1.5">
                  <span className="text-muted-foreground">Entity</span>
                  <span className="font-medium font-mono">{detail.entity}</span>
                </div>
                <div className="flex justify-between border-b border-border py-1.5">
                  <span className="text-muted-foreground">Result</span>
                  <span className="font-medium">{detail.result}</span>
                </div>
                {detail.detail && (
                  <div className="pt-1">
