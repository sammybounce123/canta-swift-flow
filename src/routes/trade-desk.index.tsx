...
import {
  createDraftTradeFile,
  readDraftTradeFiles as readDrafts,
  type TradeFileEvent,
} from "@/lib/trade-file-auto";
import {
  FileText,
  Plus,
  Search,
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Create a Trade File to pay any supplier globally. Add supplier bank details, upload
            invoice documents, accept an FX quote, fund in NGN or USDT, and track settlement. Your
            supplier does not need a Canta account.
          </p>
        </div>
...
const QUICK_STARTS: QuickStart[] = [
  {
    ...
    icon: Upload,
    event: "bl_upload",
    hint: "Drop a BL and we open a Trade File with the shipment already attached.",
  },
  {
    key: "invoice",
    ...
    hint: "Attach a supplier invoice to draft a file with payment and cost lined up.",
  },
  {
    key: "track",
    label: "Track Shipment",
    icon: Ship,
    event: "container_document",
    hint: "Enter a container or booking to start a file that tracks the movement.",
  },
  {
    key: "pay",
    label: "Start Supplier Payment",
    icon: Send,
    event: "supplier_payment_request",
    hint: "Open a payment request — the Trade File follows the money.",
  },
];
...
function NewTradeFileDialog({ open, setOpen }: { open: boolean; setOpen: (o: boolean) => void }) {
  ...
  const submit = () => {
    ...
    setOpen(false);
    toast.success("Trade File created", {
      description: `${id} — supplier beneficiary saved. No supplier account needed.`,
    });
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Pay any supplier globally. Add supplier details and bank account — your supplier does
            not need a Canta account.
          </p>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Supplier company name *">
            <Input
              value={d.supplier}
              onChange={(e) => setD({ ...d, supplier: e.target.value })}
              placeholder="Shenzhen Hua Tech Co."
            />
          </Field>
          <Field label="Supplier country *">
            <Input
              value={d.supplierCountry}
              onChange={(e) => setD({ ...d, supplierCountry: e.target.value })}
              placeholder="China"
            />
          </Field>
          <Field label="Supplier contact name">
            <Input
              ...
              onChange={(e) => setD({ ...d, supplierContact: e.target.value })}
              placeholder="Li Wei"
            />
          </Field>
          <Field label="Supplier email / WhatsApp (optional)">
            <Input
              value={d.supplierEmail}
              onChange={(e) => setD({ ...d, supplierEmail: e.target.value })}
              placeholder="li@huatech.cn"
            />
          </Field>
          ...
          <Field label="Bank account / IBAN *">
            <Input
              value={d.bankAccount}
              onChange={(e) => setD({ ...d, bankAccount: e.target.value })}
              placeholder="6210 **** 1144"
            />
          </Field>
          <Field label="SWIFT / routing">
            <Input
              value={d.swift}
              onChange={(e) => setD({ ...d, swift: e.target.value })}
        ...
        <div className="text-[11px] uppercase tracking-widest text-muted-foreground pt-1">
          Invoice & documents
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Trade File name">
            <Input
              value={d.name}
              onChange={(e) => setD({ ...d, name: e.target.value })}
              placeholder="Guangzhou Q3 Electronics"
            />
          </Field>
          <Field label="Invoice amount">
            <Input
              ...
              onChange={(e) => setD({ ...d, invoiceValue: e.target.value })}
              placeholder="184000"
            />
          </Field>
          <Field label="Goods / service description" wide>
            <Input
              value={d.goods}
              onChange={(e) => setD({ ...d, goods: e.target.value })}
              placeholder="240 cartons of mixed electronics"
            />
          </Field>
          <Field label="Invoice upload">
            <label className="flex items-center gap-2 rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground cursor-pointer hover:border-primary hover:bg-primary/5">
              ...
              <Input
                type="file"
                className="hidden"
                onChange={(e) => setInvoiceFile(e.target.files?.[0]?.name ?? "invoice.pdf")}
              />
            </label>
          </Field>
          <Field label="Supporting documents">
            <label className="flex items-center gap-2 rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground cursor-pointer hover:border-primary hover:bg-primary/5">
              ...
              <span className="truncate">
                {supportDocs.length
                  ? `${supportDocs.length} file(s) attached`
                  : "Proforma, PO, BL, packing list"}
              </span>
              <Input
                type="file"
                multiple
                className="hidden"
                onChange={(e) =>
                  setSupportDocs(Array.from(e.target.files ?? []).map((f) => f.name))
                }
              />
            </label>
          </Field>
          <Field label="Compliance purpose">
            <Select
              value={d.compliancePurpose}
              onValueChange={(v) => setD({ ...d, compliancePurpose: v })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <Field label="Notes" wide>
            <Input
              ...
              placeholder="Anything Canta compliance should know"
            />
          </Field>
        </div>
