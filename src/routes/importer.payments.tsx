  ...
  head: () => ({
    meta: [
      { title: "Supplier Payments — Canta Importer" },
      {
        name: "description",
        content:
          "Start a new supplier payment, follow pending payments, review completed payments and download receipts.",
      },
...
function PaymentsPage() {
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Payments are reviewed before payout. Records here are illustrative."
      />
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
          Pay any supplier globally. Your supplier does not need a Canta account. Start a payment,
          follow its progress, and download the receipt once the payout is confirmed.
        </p>
      </header>
            ...
            {receipts.map((p) => (
              <Card key={p.id} className="p-4 shadow-card">
                ...
                <dl className="mt-3 text-xs space-y-1">
                  ...
                  <Line
                    k="Amount supplier received"
                    v={`${p.currency} ${p.amount.toLocaleString()}`}
                  />
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => toast.success("Receipt opened")}
                  >
                    View receipt
                  </Button>
    ...
    <Card className="p-3 shadow-card">
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Search supplier, reference or status..."
          className="pl-9"
        />
      </div>
    </Card>
...
function PaymentGrid({ rows, empty }: { rows: SupplierPayment[]; empty: string }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      {rows.map((p) => (
        <Card key={p.id} className="p-4 shadow-card">
          ...
          <div className="mt-3 text-xs text-muted-foreground space-y-0.5">
            ...
            <div>
              You pay: <b className="text-foreground">{fmtNGN(p.ngnCost)}</b> (fee {fmtNGN(p.fee)})
            </div>
            <div>
              Supplier bank account: {p.bank} · {p.accountNumber}
            </div>
            <div>Documents linked: {p.documents.length || "none yet"}</div>
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className="text-xs">
              Next: <b>{NEXT_ACTION[p.status]}</b>
            </span>
            <div className="ml-auto flex gap-1.5">
              <Button asChild size="sm" variant="outline">
                <Link to="/importer/documents">Documents</Link>
              </Button>
              <Button asChild size="sm" variant="ghost">
                <Link to="/importer/shipments">Shipments</Link>
              </Button>
            </div>
          </div>
        </Card>
      ...
      {rows.length === 0 && (
        <div className="col-span-full text-center text-sm text-muted-foreground py-10">{empty}</div>
      )}
    </div>
  );
}
