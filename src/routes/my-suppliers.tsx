...
type Supplier = {
  ...
  contact: string; email: string; phone: string; terms: string; notes: string;
};

const SEED: Supplier[] = [
  { id: "SUP-001", name: "Yiwu Fashion Co.", country: "China", category: "Apparel", contact: "Mei Lin", email: "mei@yiwufashion.cn", phone: "+86 579 0001", terms: "30% TT, 70% on BL", notes: "Reliable, 12 shipments" },
  { id: "SUP-002", name: "Guangzhou Electronics", country: "China", category: "Electronics", contact: "Wei Zhang", email: "wei@gze.cn", phone: "+86 20 0101", terms: "50/50 TT", notes: "" },
  { id: "SUP-003", name: "Istanbul Textiles", country: "Turkey", category: "Textiles", contact: "Ahmet K.", email: "ahmet@istex.tr", phone: "+90 212 0101", terms: "Net 45", notes: "" },
];
...
function MySuppliersPage() {
  ...
  const filtered = useMemo(() =>
    list.filter((s) => !q || s.name.toLowerCase().includes(q.toLowerCase()) || s.country.toLowerCase().includes(q.toLowerCase()) || s.category.toLowerCase().includes(q.toLowerCase())),
    [list, q],
  );
    ...
    <div className="space-y-6">
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-1.5" /> Add supplier</Button>
          </DialogTrigger>
      ...
      <Card className="p-3 shadow-card">
        <div className="relative">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by name, country, or category..." className="pl-9" />
        </div>
      </Card>
          ...
          <Card key={s.id} className="p-4 shadow-card">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><MapPin className="h-3 w-3" />{s.country} · {s.category}</div>
              </div>
              <Badge variant="outline" className="text-[10px] shrink-0">{s.id}</Badge>
            </div>
            ...
            <div className="mt-3 text-[11px] bg-secondary/40 rounded p-2"><b>Terms:</b> {s.terms}</div>
            {s.notes && <div className="mt-2 text-[11px] text-muted-foreground">{s.notes}</div>}
          </Card>
        ))}
        {filtered.length === 0 && (
          <div className="col-span-full text-center text-sm text-muted-foreground py-10">No suppliers match.</div>
        )}
      </div>
    </div>
  );
...
function AddSupplierDialog({ onSubmit, onClose }: { onSubmit: (d: Omit<Supplier, "id">) => void; onClose: () => void }) {
  const [f, setF] = useState({ name: "", country: "", category: "", contact: "", email: "", phone: "", terms: "", notes: "" });
  ...
  return (
    <DialogContent className="max-w-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div><Label>Phone</Label><Input value={f.phone} onChange={set("phone")} placeholder="+86 ..." /></div>
        <div><Label>Payment terms</Label><Input value={f.terms} onChange={set("terms")} placeholder="30% TT, 70% on BL" /></div>
        <div className="sm:col-span-2"><Label>Notes</Label><Textarea value={f.notes} onChange={set("notes")} placeholder="History, references..." /></div>
      </div>
      <DialogFooter>
