  ...
  head: () => ({
    meta: [
      { title: "Settings — Supplier Portal — Canta" },
      {
        name: "description",
        content:
          "Language, automatic convert default, notifications and settlement currency for your supplier account.",
      },
    ],
  }),
    ...
    <div className="space-y-3">
      ...
      <Card className="space-y-2 p-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <Switch
            checked={autoConvert}
            aria-label="Automatic Convert default"
            onCheckedChange={(v) => {
              autoConvertStore.set(v);
              toast.success(v ? "Default set to ON" : "Default set to OFF");
            }}
          />
        </div>
      </Card>
        ...
        {(
          ...
          <div key={k} className="flex items-center justify-between text-sm">
            ...
            <Switch
              checked={channels[k]}
              aria-label={label}
              onCheckedChange={(v) => setChannels((c) => ({ ...c, [k]: v }))}
            />
          </div>
        ))}
        <Button
      ...
      <Card className="space-y-2 p-4">
        ...
        <p className="text-xs text-muted-foreground">
          Settlement is only ever paid to your verified bank account. You cannot send funds out of
          Canta.
        </p>
        <Button size="sm" variant="outline" asChild>
          <Link to="/supplier-portal/rmb-bank-account">Manage bank accounts</Link>
        </Button>
      </Card>
      ...
      <Card className="space-y-2 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            size="sm"
            variant="outline"
            onClick={() => toast.info("Password change link sent to your email")}
          >
            Change password
          </Button>
      ...
      <Card className="space-y-2 p-4">
        ...
        <div className="text-xs text-muted-foreground">
          Business licence 91440101MA9XK2R37D · Consumer electronics
        </div>
        <div className="flex flex-wrap gap-2 pt-1">
          <Button size="sm" variant="outline" asChild>
            <Link to="/supplier-portal/profile">View full profile</Link>
          </Button>
          <Button size="sm" variant="outline" asChild>
            <Link to="/supplier-portal/verification">{t("verification")}</Link>
          </Button>
        </div>
      </Card>
    </div>
