...
function SettingsPage() {
  ...
  const s = getSettings();
  const bind = (k: keyof PartnerSettings) => ({
    checked: !!s[k],
    onCheckedChange: (v: boolean) => setSetting(k, v),
  });
    ...
    <div className="space-y-5 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Workspace, permissions, branding and commission policy for {PARTNER_ORG.name}.
        </p>
      </div>
      ...
      <Card className="p-6 shadow-card space-y-4">
        <div className="text-sm font-semibold flex items-center gap-2">
          <ShieldCheck className="h-4 w-4 text-primary" /> Permissions &amp; visibility
        </div>
        <Toggle label="Hide client BVN from partner users" {...bind("hideBVNFromPartner")} />
        <Toggle
          label="Require admin approval to edit solicitor bank details"
          {...bind("requireAdminApprovalForBankEdit")}
        />
      </Card>

      <Card className="p-6 shadow-card space-y-4">
        <div className="text-sm font-semibold flex items-center gap-2">
          <Percent className="h-4 w-4 text-primary" /> Commissions
        </div>
        <Toggle
          label="Enable commission tracking (adds Commissions to the sidebar)"
          {...bind("commissionsEnabled")}
        />
        <Toggle
          label="Marketers can view their own attributed commission"
          {...bind("marketerSeesOwnCommission")}
        />
      </Card>
      ...
      <Card className="p-6 shadow-card space-y-4">
        ...
        <Row
          label="Payment page header"
          value="Canta × Kingsbridge Property Partners Property Payment"
        />
        <Row label="Display referring partner" value="Kingsbridge Property Partners" />
        <Badge variant="outline" className="text-[10px]">
          Premium property-focused branding applied
        </Badge>
      </Card>
    </div>
  );
