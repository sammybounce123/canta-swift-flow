...
export const Route = createFileRoute("/pay/$linkId")({
  head: () => ({
    meta: [{ title: "Secure Property Payment — Canta × Kingsbridge Property Partners" }],
  }),
...
function ClientPayPage() {
  ...
  const linkCompleted =
    c.paymentLink?.status === "Completed" ||
    c.payout?.status === "Paid to Solicitor" ||
    c.payout?.status === "Receipt Uploaded";
      ...
      <div className="mx-auto max-w-3xl px-4 pb-16 space-y-5">
        <Card className="p-5 shadow-card flex flex-wrap items-center gap-3 bg-gradient-to-br from-primary/5 to-accent/5 border-primary/20">
          ...
          <div className="text-sm">
            ...
            <span className="text-muted-foreground">
              {" "}
              referred this payment · Canta will process FX and pay your UK solicitor.
            </span>
          </div>
          {quote && !quoteExpired && (
        ...
        {linkCompleted ? (
          ...
          <>
            ...
            {step === "done" && (
              <DoneStep caseId={c.id} sol={sol?.firm ?? "your solicitor"} amount={c.amountGBP} />
            )}
          </>
        )}
      </div>
    ...
    <div className="flex items-center gap-2 text-xs flex-wrap">
      {steps.map((s, i) => (
        <div key={s.id} className="flex items-center gap-2">
          <div
            className={`h-6 w-6 rounded-full grid place-items-center font-semibold ${i <= idx ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            {i + 1}
          </div>
...
function ReviewStep({ c, quote, sol, onNext }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  return (
    <Card className="p-6 shadow-card space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <Row
          label="Exchange rate"
          value={quote ? `1 GBP = ₦${quote.rate.toLocaleString()}` : "—"}
        />
        <Row label="Canta fee" value={quote ? formatGBP(quote.feeGBP) : "—"} />
        <Row
          label="NGN you pay"
          value={quote ? `₦${quote.ngnTotal.toLocaleString()}` : "—"}
          highlight
        />
      </div>
...
function VerifyStep({ caseId, clientName, onDone }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const valid =
    bvn.length === 11 && dob && name && src && purpose && canta && shared && terms && privacy;
  const submit = () => {
    submitVerification(caseId, {
      bvn,
      dob,
      fullNameConfirmed: name,
      sourceOfFunds: src,
      consent: { propertyPurpose: purpose, canta, sharedDocs: shared, terms, privacy },
    });
    appendDocAudit({
      caseId,
      docType: "Consent",
      action: "Document consent completed",
      actorId: "client",
    ...
    <Card className="p-6 shadow-card space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          A quick check before we show you the Canta funding account.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <Input
            ...
            placeholder="e.g. Personal savings / business income / property sale"
          />
        </div>
      </div>
      <div className="space-y-2 text-sm">
        <ConsentRow
          checked={name}
          onChange={setName}
          label={`I confirm my full name matches: ${clientName}`}
        />
        <ConsentRow
          checked={canta}
          onChange={setCanta}
          label="I consent to Canta processing this property payment."
        />
        <ConsentRow
          checked={shared}
          onChange={setShared}
          label="I consent to Canta using KYC documents shared by Kingsbridge Property Partners for verification."
        />
        <ConsentRow
          checked={purpose}
          onChange={setPurpose}
          label="I confirm the payment purpose and source of funds are accurate."
        />
        <ConsentRow
          checked={terms}
          onChange={setTerms}
          label="I accept Canta's Terms of Service."
        />
        <ConsentRow
          checked={privacy}
          onChange={setPrivacy}
          label="I accept Canta's Privacy and Data Processing Policy."
        />
      </div>
...
function DocStep({ c, confirmed, setConfirmed, onNext }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  return (
    <Card className="p-6 shadow-card space-y-4">
      <div className="text-sm font-semibold flex items-center gap-2">
      ...
      {c.documents.length > 0 ? (
        <>
          ...
          <ul className="text-sm border rounded-lg divide-y">
            {c.documents.map((d: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
              <li
                key={d.id}
        ...
        <div className="text-sm text-muted-foreground">
          No documents on file yet — you can upload any required documents below.
        </div>
      )}
      ...
      <ConsentRow
        ...
        label="I confirm all required documents have been provided or uploaded."
      />
      <div className="flex justify-end">
...
function FundGate({ c, quote, docsConfirmed, onPaid }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
      ...
      <Card className="p-6 shadow-card space-y-4 border-warning/30">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Complete the items below before Canta shows you the funding account details.
            </p>
          </div>
        </div>
...
function FundStep({ c, quote, onPaid }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  const copy = (s: string) => {
  ...
  const submit = () => {
    ...
    else if (payerName.toLowerCase() !== c.clientName.toLowerCase())
      toast.warning("Marked as Name Mismatch for review");
    else if (Math.abs(Number(amount) - quote.ngnTotal) / quote.ngnTotal > 0.005)
      toast.warning("Marked as Amount Mismatch for review");
  ...
  return (
    <Card className="p-6 shadow-card space-y-4">
      ...
      <div className="rounded-lg border bg-secondary/30 p-4 grid grid-cols-2 gap-3 text-sm">
        ...
        <Row
          label="Amount"
          value={`₦${quote.ngnTotal.toLocaleString()}`}
          highlight
          copyable
          onCopy={() => copy(String(quote.ngnTotal))}
        />
        <Row
          label="Payment reference"
          value={quote.reference}
          copyable
          onCopy={() => copy(quote.reference)}
        />
      ...
      <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 text-xs">
        Only pay into the Canta account details shown on this secure page. Pay from an account in
        your own name where possible. Canta will process FX conversion and the solicitor payout
        after funding and compliance checks.
      </div>
      ...
      <div className="border-t pt-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            <Label className="text-xs">Payer name on transfer</Label>
            <Input value={payerName} onChange={(e) => setPayerName(e.target.value)} />
          </div>
          <div>
            <Label className="text-xs">Reference used</Label>
            <Input value={reference} onChange={(e) => setReference(e.target.value)} />
          </div>
        </div>
      </div>
      <div className="flex justify-end">
        <Button onClick={submit}>I have made the payment</Button>
      </div>
    </Card>
  );
}
    ...
    <Card className="p-8 shadow-card text-center space-y-4">
      ...
      <div>
        <div className="text-lg font-semibold">
          Thank you — your property payment is being processed
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          Canta will pay {formatGBP(amount)} to {sol}. You'll receive a receipt once settled. If any
          of your payment details didn't match exactly, our compliance team will reach out before
          processing.
        </p>
      </div>
      <div className="border-t pt-4 space-y-2 text-left max-w-md mx-auto">
        <div className="text-sm font-semibold flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-primary" /> Activate your full Canta account
        </div>
        <p className="text-sm text-muted-foreground">
          Use Canta for future international payments, treasury workflows and global transfers.
        </p>
        <div className="flex gap-2">
          <Button asChild className="flex-1">
            <Link to="/welcome" onClick={() => activateClientAccount(caseId)}>
              Activate Canta account
            </Link>
          </Button>
...
function Row({
  ...
  return (
    <div>
      ...
      <div
        className={`mt-0.5 flex items-center gap-1.5 ${highlight ? "text-lg font-semibold tabular-nums" : "text-sm font-medium"}`}
      >
        {value}
        {copyable && (
