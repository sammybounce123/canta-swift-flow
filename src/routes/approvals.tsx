...
const seed: Request[] = [
  {
    ...
    due: "Jun 9, 17:00",
    comments: [{ who: "Compliance Bot", when: "Jun 8, 09:14", note: "Sanctions screening clear." }],
  },
  {
    id: "APR-9820",
  ...
  },
  {
    ...
    requester: "Femi Adeyemi",
    customer: "Yiwu Fashion Trading",
    riskScore: 35,
    documents: ["KYB Pack", "Bank Confirmation"],
    status: "Pending",
    submitted: "Jun 7, 16:20",
    due: "Jun 9, 12:00",
    comments: [],
  },
  {
    ...
    currency: "USD",
    riskScore: 18,
    documents: ["Rate Quote"],
    status: "Pending",
    submitted: "Jun 8, 07:55",
    due: "Jun 8, 14:00",
    overdue: true,
    comments: [],
  },
  {
    id: "APR-9814",
    type: "Expense policy update",
    requester: "Chiamaka Eze",
    customer: "Marketing Team · Lagos",
    riskScore: 12,
    documents: ["Expense Policy Ack"],
    status: "Pending",
    submitted: "Jun 8, 11:02",
    due: "Jun 10, 17:00",
    comments: [],
  },
  {
    id: "APR-9810",
    type: "Freight invoice approval",
    requester: "Ibrahim Lawal",
    amount: 18_400,
    currency: "USD",
  ...
  {
    id: "APR-9805",
    type: "Global collection settlement",
    requester: "Merchant Admin",
    amount: 412_000,
    currency: "USD",
    customer: "Pan-African University",
    riskScore: 31,
    documents: ["Reconciliation Report"],
    status: "Pending",
    submitted: "Jun 8, 06:30",
    due: "Jun 9, 17:00",
    comments: [],
  },
  {
  ...
  },
  {
    ...
    requester: "Canta Trade Officer",
    customer: "New Importer · Lagos Hardware",
    riskScore: 78,
    documents: ["CAC", "MEMART", "UBO Form"],
    status: "Pending",
    submitted: "Jun 8, 09:50",
    due: "Jun 9, 17:00",
    comments: [
      {
        who: "Compliance Bot",
        when: "Jun 8, 09:52",
        note: "PEP screening flag — enhanced DD recommended.",
      },
    ],
  },
  {
  ...
  {
    id: "APR-9788",
    type: "Marketing budget approval",
    requester: "Femi Adeyemi",
    customer: "Online Ads · Q3",
    riskScore: 9,
    documents: [],
    status: "Approved",
    submitted: "Jun 7, 10:00",
    due: "Jun 8, 17:00",
    comments: [],
  },
  {
    ...
    type: "New beneficiary",
    requester: "Procurement Officer",
    customer: "Suspicious Holdings Ltd",
    riskScore: 82,
    documents: ["KYB Pack"],
    status: "Rejected",
    submitted: "Jun 7, 09:00",
    due: "Jun 8, 17:00",
    comments: [
      { who: "Chiamaka Eze", when: "Jun 7, 14:20", note: "Failed adverse media screening." },
    ],
  },
];
...
function Approvals() {
  ...
  const canApprove =
    !profile ||
    profile.permissions.includes("approve payment") ||
    profile.permissions.includes("approve settlement") ||
    profile.role.toLowerCase().includes("admin") ||
    profile.role.toLowerCase().includes("owner") ||
    profile.role.toLowerCase().includes("officer");
  ...
  const filtered = useMemo(
    () =>
      items.filter((r) =>
        `${r.id} ${r.type} ${r.requester} ${r.customer ?? ""}`
          .toLowerCase()
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Approvals enforce dual control on high-value actions."
      />
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Approvals</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Cross-product approval workflows for trade, payments, escrow, FX and compliance.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered
              ...
              .map((r) => (
                <Card
                  key={r.id}
                  className="p-5 shadow-card hover:shadow-elegant transition cursor-pointer"
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant="outline" className={`${riskTone(r.riskScore)} text-[10px]`}>
                      Risk {r.riskScore}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4 text-xs">
                    ...
                    <div>
                      ...
                      <div className={`font-medium mt-0.5 ${r.overdue ? "text-destructive" : ""}`}>
                        {r.due}
                        {r.overdue && " · overdue"}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                    <Badge variant="outline" className={`${statusTone(r.status)} text-[10px]`}>
                      {r.status}
                    </Badge>
                    <div
                      ...
                      title={
                        !canApprove
                          ? "You do not have approval permission for this workspace"
                          : undefined
                      }
                    >
                      <Button
              ...
              <Card key={w.type} className="p-5 shadow-card">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {w.steps.length}-step approval chain
                    </div>
                  </div>
                ...
                <div className="flex gap-2 mt-5">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => toast.success(`${w.type} workflow edited`)}
                  >
                    Edit chain
                  </Button>
                  <Button
                  ...
                  .map((r) => (
                    <tr key={r.id} className="border-t border-border hover:bg-secondary/30">
                      ...
                      <td className="px-5 py-3 tabular-nums">
                        {r.amount ? `${r.currency} ${r.amount.toLocaleString()}` : "—"}
                      </td>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <Detail
                  ...
                  value={
                    openItem.amount
                      ? `${openItem.currency} ${openItem.amount.toLocaleString()}`
                      : "—"
                  }
                />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {openItem.documents.map((d) => (
                    <Badge key={d} variant="outline" className="border-border text-[11px]">
                      {d}
                    </Badge>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  ...
                  {openItem.comments.map((c, i) => (
                    <div
                      key={i}
                      className="text-xs p-2 rounded bg-secondary/40 border border-border"
                    >
                      <div className="font-medium">
                        {c.who}{" "}
                        <span className="text-muted-foreground font-normal">· {c.when}</span>
                      </div>
              ...
              {!canApprove && (
                <div className="text-xs text-amber-700 bg-amber-500/10 border border-amber-500/30 rounded-md px-3 py-2">
                  Your role does not include approval permission. Approve/Reject are disabled —
              ...
              <DialogFooter className="gap-2">
                <Button
                  variant="outline"
                  onClick={() =>
                    act(openItem.id, "Info Requested", comment || "More information requested")
                  }
                >
                ...
                <Button
                  variant="outline"
                  disabled={!canApprove}
                  title={!canApprove ? "You do not have approval permission" : undefined}
                  className="border-destructive/30 text-destructive hover:bg-destructive/10"
                  onClick={() => act(openItem.id, "Rejected", comment || "Rejected")}
                >
                ...
                <Button
                  ...
                  title={!canApprove ? "You do not have approval permission" : undefined}
                  className="bg-success text-success-foreground hover:bg-success/90"
                  onClick={() => act(openItem.id, "Approved", comment || "Approved")}
                >
