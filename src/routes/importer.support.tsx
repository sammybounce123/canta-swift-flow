  ...
  head: () => ({
    meta: [
      { title: "Importer Support — Canta" },
      {
        name: "description",
        content:
          "Help with supplier payments, funding, FX quotes, documents, BL uploads, shipment tracking, receipts and WhatsApp updates.",
      },
...
const TOPICS: Topic[] = [
  {
    key: "payment",
    title: "Supplier payment issue",
    desc: "A payment is stuck, delayed or sent with the wrong details.",
    icon: Send,
  ...
  },
  {
    ...
    desc: "You funded your balance but it has not been credited yet.",
    icon: Wallet,
    issueType: "Funding mismatch",
  },
  {
    key: "fx",
    title: "FX quote expired",
    desc: "Your rate expired before you accepted it and you need a new quote.",
    icon: RefreshCw,
    issueType: "Payment issue",
  },
  {
    key: "bank",
    title: "Supplier bank details correction",
    desc: "Correct a supplier account number, SWIFT code or bank name.",
    icon: Banknote,
    issueType: "Payment issue",
  },
  {
    key: "upload",
    title: "Upload invoice or BL",
    desc: "You cannot upload an invoice, packing list or Bill of Lading.",
    icon: Upload,
    issueType: "Technical issue",
  },
  {
    key: "shipment",
    title: "Shipment tracking / WhatsApp bot",
    desc: "Tracking is not updating or the WhatsApp bot is not replying.",
    icon: Ship,
    issueType: "Shipment issue",
  },
  {
    key: "receipt",
    title: "Receipt or settlement confirmation",
    desc: "You need proof of payment or confirmation the supplier was paid.",
    icon: Receipt,
    issueType: "Payment issue",
  },
  {
    key: "compliance",
    title: "Compliance review / missing documents",
    desc: "Your payment is under review or a document was requested.",
    icon: ShieldCheck,
    issueType: "KYC/KYB issue",
  },
];
...
function ImporterSupportPage() {
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Support tickets created here are illustrative and stay inside this demo."
      />
      ...
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground max-w-3xl">
          Get help with supplier payments, funding, FX quotes, documents, BL uploads, shipment
          tracking, receipts, and WhatsApp updates.
        </p>
      </header>
      ...
      <Card className="p-4 shadow-card">
        <div className="flex flex-wrap items-center gap-2">
          <TicketDialog
            topic={openTopic}
            onOpenChange={(v) => {
              if (!v) setOpenTopic(null);
            }}
            business={s.business}
            trigger={
              <Button>
                <Ticket className="h-4 w-4" /> Create support ticket
              </Button>
            }
          />
          ...
          {WHATSAPP_SUPPORT_CONFIGURED ? (
            <Button
              ...
              onClick={() =>
                window.open(
                  `https://wa.me/${CANTA_WHATSAPP_NUMBER}?text=${encodeURIComponent("Hello Canta support, I need help with my supplier payment.")}`,
                  "_blank",
                  "noopener",
                )
              }
          ...
          <Button variant="ghost" asChild className="sm:ml-auto">
            <a href="#previous-tickets">
              View previous tickets{openCount ? ` (${openCount} open)` : ""}
            </a>
          </Button>
        </div>
      </Card>
            ...
            <Card key={t.id} className="p-3 shadow-card flex flex-wrap items-center gap-2">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  Opened {t.createdAt}
                  {t.linkedRef ? ` · linked to ${t.linkedRef}` : ""}
                </div>
              </div>
      ...
      <Card className="p-4 shadow-card text-xs text-muted-foreground">
        Quick links:{" "}
        <Link to="/importer/payments" className="text-primary underline">
          Payments
        </Link>{" "}
        ·{" "}
        <Link to="/importer/shipments" className="text-primary underline">
          Shipments
        </Link>{" "}
        ·{" "}
        <Link to="/importer/documents" className="text-primary underline">
          Documents
        </Link>{" "}
        ·{" "}
        <Link to="/importer/balance" className="text-primary underline">
          Balance
        </Link>
      </Card>
    </div>
...
function TicketDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={close}>
      <DialogTrigger asChild onClick={() => setManualOpen(true)}>
        {trigger}
      </DialogTrigger>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. SP-2026-0140 or MSCUNG2291"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="Tell us what happened so we can help faster."
            />
          </div>
        </div>
        <DialogFooter>
          ...
          <Button
            onClick={() => {
              ...
              toast.success(`Ticket ${t.ref} created`, {
                description: "Canta support will reply in this demo workspace.",
              });
              close(false);
            }}
          >
