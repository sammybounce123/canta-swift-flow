...
export const Route = createFileRoute("/trade-desk/$fileId")({
  ...
  notFoundComponent: () => (
    <div className="p-10 text-center text-sm text-muted-foreground">
      ...
      <Link to="/trade-desk" className="text-primary underline">
        Back to Trade Desk
      </Link>
    </div>
  ),
...
function TradeFileDetail() {
  ...
  const file: (typeof tradeFiles)[number] =
    known ??
    ({
      id: fileId,
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Documents remain linked to this transaction for audit and reference."
      />
      {/* Header */}
      <div>
        <Link
        ...
        <Card className="p-4 shadow-card">
          <div className="text-xs uppercase tracking-widest text-muted-foreground mb-3">
            Quick actions on this trade file
          </div>
          <TradeFileActions fileId={file.id} supplier={file.supplier} onNavigate={navigate} />
        </Card>
      </div>

      <Card className="p-4 shadow-card border-accent/20 bg-gradient-to-br from-accent/5 to-transparent">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground mt-1 max-w-xl">
              You pay in NGN or USDT and Canta settles your supplier's own bank account. Supplier
              does not need a Canta account.
            </div>
          </div>
          <ButtonGroup
            label="Supplier beneficiary actions"
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-3 text-xs">
          ...
          <Field
            ...
            value={(file as { supplierCountry?: string }).supplierCountry ?? file.origin}
          />
          <Field
            label="Beneficiary bank"
            value={(file as { bankName?: string }).bankName ?? "On file"}
          />
          <Field label="Invoice amount" value={fmtMoney(file.invoiceValue, file.ccy)} />
          <Field
            label="NGN equivalent"
            value={`₦${Math.round(file.invoiceValue * 1612).toLocaleString()}`}
          />
          <Field
            label="Settlement currency"
            value={(file as { settlementCcy?: string }).settlementCcy ?? file.ccy}
          />
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-background/70 p-3">
          <div className="text-xs text-muted-foreground">
            Payment path: Trade File created → Supplier beneficiary added → Invoice uploaded → FX
            quote accepted → Funded in NGN / USDT → Compliance review → Supplier receives settlement
            → Receipt available
          </div>
          <Button
            size="sm"
            onClick={() => toast.success("Supplier payment initiated — funding in NGN")}
          >
            Pay Supplier
          </Button>
        </div>
      ...
      <Tabs defaultValue="overview" className="space-y-5">
        ...
        <TabsContent value="overview" className="space-y-5">
          <Card className="p-6 shadow-card">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
              ...
              <Field
                label="Invoice value"
                value={fmtMoney(file.invoiceValue, file.ccy)}
                highlight
              />
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <Card className="p-5 shadow-card lg:col-span-2 border-accent/20 bg-gradient-to-br from-accent/5 to-transparent">
              ...
              <div className="text-sm text-muted-foreground mt-1">
                ETA {file.eta} · Current stage:{" "}
                <span className="font-medium text-foreground">
                  {shipmentMilestones[reachedIdx - 1]}
                </span>
              </div>
              <ButtonGroup label="Next action controls" className="mt-4">
                <Button size="sm" onClick={() => toast.success("Action assigned")}>
                  Assign to teammate
                </Button>
                <Button size="sm" variant="outline" onClick={() => toast.success("Reminder set")}>
                  Set reminder
                </Button>
              </ButtonGroup>
            </Card>
            <Card className="p-5 shadow-card">
              ...
              <div className="mt-3 space-y-2 text-sm">
                ...
                <ReadyRow
                  label="Form M / SONCAP"
                  ok={file.status === "Cleared" || file.status === "Delivered"}
                />
                <ReadyRow
                  label="Duty paid"
                  ok={file.status === "Cleared" || file.status === "Delivered"}
        ...
        <TabsContent value="timeline">
          <Card className="p-6 shadow-card">
            <div className="text-sm font-semibold flex items-center gap-2">
              <Activity className="h-4 w-4" /> Shipment timeline
            </div>
              ...
              {shipmentMilestones.map((m, i) => {
                ...
                return (
                  <div key={m} className="flex items-start gap-3">
                    <div
                      className={`mt-0.5 h-6 w-6 rounded-full grid place-items-center text-[10px] font-bold flex-shrink-0 ${
                        current
        ...
        <TabsContent value="escrow">
          <Card className="p-6 shadow-card">
            <div className="text-sm font-semibold flex items-center gap-2">
              <Shield className="h-4 w-4" /> Escrow protection
            </div>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-lg border border-border bg-secondary/30">
                ...
                <div className="text-[11px] text-muted-foreground mt-1">
                  Funds held by Canta until milestones clear
                </div>
              </div>
              <div className="p-4 rounded-lg border border-border bg-secondary/30">
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                ...
                <div className="mt-2 text-lg font-semibold tabular-nums">
                  {fmtMoney(file.invoiceValue, file.ccy)}
                </div>
                <div className="text-[11px] text-muted-foreground mt-1">
                  Released against BL + delivery proof
                </div>
              </div>
              <div className="p-4 rounded-lg border border-border bg-secondary/30">
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                ...
                <div className="text-[11px] text-muted-foreground mt-1">
                  Open dispute window: 7 days post-delivery
                </div>
              </div>
            </div>
            <div className="mt-5 space-y-2">
              {[
                ...
                {
                  l: "Customs cleared",
                  done: file.status === "Cleared" || file.status === "Delivered",
                },
            ...
            <ButtonGroup label="Escrow actions" className="mt-5">
              <Button size="sm" onClick={() => toast.success("Escrow release requested")}>
                Request release
              </Button>
              <Button size="sm" variant="outline" onClick={() => toast.success("Dispute opened")}>
                Open dispute
              </Button>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => toast.info("Funds secured certificate downloaded")}
              >
        ...
        <TabsContent value="whatsapp">
          <Card className="p-6 shadow-card">
            <div className="text-sm font-semibold flex items-center gap-2">
              <MessageCircle className="h-4 w-4 text-[#25D366]" /> WhatsApp conversation history
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              All messages between the importer, supplier, freight and Canta agent for {file.id}
            </div>
            <div className="mt-5 space-y-3">
              {[
                {
                  ...
                  side: "in",
                  t: "Mon 9:14 AM",
                  m: "Pls confirm vessel ETA for my container.",
                },
                {
                  who: "Canta Agent",
                  side: "out",
                  t: "Mon 9:18 AM",
                  m: `Hi, vessel for ${file.id} is on schedule. ETA ${file.eta}. I'll send BL once forwarder uploads it.`,
                },
                {
                  who: "Forwarder",
                  side: "in",
                  t: "Tue 11:02 AM",
                  m: "BL uploaded. Customs prep started.",
                },
                {
                  who: "Canta Bot",
                  side: "out",
                  t: "Tue 11:03 AM",
                  m: "📄 BL automatically attached to trade file.",
                },
                {
                  who: "Supplier",
                  side: "in",
                  t: "Wed 2:31 PM",
                  m: "Balance payment received via Canta escrow. Thank you.",
                },
                {
                  who: "Importer",
                  side: "in",
                  t: "Today 8:42 AM",
                  m: "How much duty should I expect?",
                },
                {
                  who: "Canta Agent",
                  side: "out",
                  t: "Today 8:45 AM",
                  m: "Estimated landed cost is in the trade file. Duty ≈ 18% of invoice value.",
                },
              ].map((msg, i) => (
                <div
                  key={i}
                ...
                >
                  <div
                    className={`max-w-[75%] p-3 rounded-2xl text-sm ${msg.side === "out" ? "bg-[#25D366]/15 border border-[#25D366]/30" : "bg-secondary/50 border border-border"}`}
                  >
            ...
            <div className="mt-5 flex gap-2">
              ...
              <Button
                onClick={() => toast.success("Message sent on WhatsApp")}
          ...
          <Card className="p-6 shadow-card">
            ...
            <div className="mt-4 space-y-3 text-sm">
              {[
                {
                  t: "2h ago",
                  e: "WhatsApp alert sent to importer about ETA update",
                  who: "Canta Bot",
                },
                {
                  t: "Yesterday",
                  e: "Bill of lading uploaded by Dragon Freight",
                  who: "Freight Forwarder",
                },
...
function TradeFileActions({
  ...
  const actions = [
    ...
    {
      label: "View Supplier Settlement",
      icon: Banknote,
    ...
    },
    {
      ...
      icon: FileCheck2,
      onClick: () => {
        onNavigate({ to: "/payments" });
        toast.success(`Supplier payment initiated for ${fileId}`);
      },
    },
    {
      ...
      icon: MessageCircle,
      onClick: () => {
        onNavigate({ to: "/whatsapp" });
        toast.success("Draft WhatsApp update ready");
      },
    },
  ];
...
function Field({
  ...
  return (
    <div>
      ...
      <div
        className={`text-sm mt-1 ${highlight ? "font-semibold text-accent text-base" : "font-medium"}`}
      >
        {value}
      </div>
    </div>
...
function Documents({ fileStatus }: { fileStatus: string }) {
  const [docs, setDocs] = useState([
    ...
    { name: "Bill of lading", uploaded: fileStatus !== "Drafting", required: true },
    {
      ...
      uploaded: fileStatus === "Arrived" || fileStatus === "Delivered",
      required: true,
    },
        ...
        {docs.map((d, i) => (
          ...
          >
            ...
            <div className="flex gap-2">
              {d.uploaded ? (
                <Button size="sm" variant="ghost" onClick={() => toast.info("Opening preview…")}>
                  View
                </Button>
              ) : (
                <Button
...
function Payments({ file }: { file: (typeof tradeFiles)[number] }) {
  ...
  const rows = [
    ...
    },
    {
      ...
      total: freight,
      paid: file.status === "Arrived" || file.status === "Delivered" ? freight : 0,
      ccy: file.ccy,
      ref: "FRT-908-21",
      status: file.status === "Arrived" || file.status === "Delivered" ? "Paid" : "Pending",
    },
    {
      label: "Duty & clearing",
      total: duty,
      paid: file.status === "Cleared" || file.status === "Delivered" ? duty : 0,
      ccy: "NGN",
      ref: "CUS-441-77",
      status: file.status === "Cleared" || file.status === "Delivered" ? "Paid" : "Pending",
    },
    {
      label: "Insurance",
      total: insurance,
  ...
  return (
    <Card className="p-6 shadow-card">
      <div className="text-sm font-semibold flex items-center gap-2">
        <FileCheck2 className="h-4 w-4" /> Payments
      </div>
            ...
            <div key={r.label} className="p-4 rounded-lg border border-border bg-secondary/30">
              ...
              <div className="grid grid-cols-3 gap-3 mt-3 text-xs">
                <div>
                  ...
                  <div className="font-semibold tabular-nums">{fmtMoney(r.total, r.ccy)}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Paid</div>
                  <div className="font-semibold tabular-nums text-success">
                    {fmtMoney(r.paid, r.ccy)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Outstanding</div>
                  <div
                    className={`font-semibold tabular-nums ${outstanding > 0 ? "text-amber-600" : ""}`}
                  >
                    {fmtMoney(outstanding, r.ccy)}
                  </div>
                </div>
              </div>
...
function LandedCost({ value }: { value: number }) {
  ...
  return (
    <Card className="p-6 shadow-card border-accent/20 bg-gradient-to-br from-accent/5 to-transparent">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="text-sm font-semibold flex items-center gap-2">
          <Calculator className="h-4 w-4 text-accent" /> Landed cost calculator
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-5">
        ...
        <div className="space-y-3">
          ...
          <Row
            label={`Expected sale (${margin}% margin)`}
            v={`₦${expectedSale.toLocaleString()}`}
          />
          ...
          <div className="p-4 rounded-lg bg-success/10 border border-success/20 text-xs mt-2">
            At <span className="font-semibold">{margin}%</span> margin, this trade returns{" "}
            <span className="font-semibold">₦{profit.toLocaleString()}</span> on landed cost of{" "}
            <span className="font-semibold">₦{totalNgn.toLocaleString()}</span>.
          </div>
          <Button
            className="w-full"
            onClick={() => toast.success("Landed cost saved to trade file")}
          >
            Save to trade file
          </Button>
        </div>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/40 border border-border">
      ...
      <div
        className={`text-sm font-semibold tabular-nums ${highlight ? "text-accent text-base" : success ? "text-success" : ""}`}
      >
        {v}
      </div>
    </div>
