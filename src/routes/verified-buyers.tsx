  ...
  const list = useMemo(
    () =>
      BUYERS.filter((b) => {
        ...
        if (q && !`${b.name} ${b.city} ${b.country}`.toLowerCase().includes(q.toLowerCase()))
          return false;
        return true;
      }),
    ...
    <div className="space-y-6">
      ...
      <header>
        <Badge variant="outline" className="gap-1">
          <ShieldCheck className="h-3 w-3" /> Canta Trade Network
        </Badge>
        <h1 className="text-2xl font-semibold tracking-tight mt-2">Verified African Buyers</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Only buyers who opted in to discovery are shown. Reach out through Canta — full contact
          info is shared once both parties accept a trade request.
        </p>
      </header>
          ...
          <button key={b.id} onClick={() => setSelected(b)} className="text-left">
            <Card className="p-4 hover:shadow-card hover:-translate-y-0.5 transition h-full">
              ...
              <div className="grid grid-cols-3 gap-2 mt-4 text-xs">
                <div>
                  <div className="text-muted-foreground">Pay score</div>
                  <div className="font-semibold">{b.paymentScore}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Done</div>
                  <div className="font-semibold">{b.completedTx}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Escrow</div>
                  <div className="font-semibold">{b.escrowHistory}</div>
                </div>
              </div>
              <div className="mt-3 text-[11px] text-muted-foreground flex items-center justify-between gap-2">
                <span className="inline-flex items-center gap-1">
                  <Wallet className="h-3 w-3" /> ${b.avgOrderUsd[0].toLocaleString()}–$
                  {b.avgOrderUsd[1].toLocaleString()}
                </span>
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                ...
                <section className="grid grid-cols-2 gap-3">
                  <StatBox
                    label="Trade history"
                    value={`${selected.tradeHistory} deals`}
                    icon={TrendingUp}
                  />
                  <StatBox
                    label="Payment score"
                    value={`${selected.paymentScore}/100`}
                    icon={BadgeCheck}
                  />
                  <StatBox
                    label="Completed tx"
                    value={String(selected.completedTx)}
                    icon={FileText}
                  />
                  <StatBox
                    label="Avg order"
                    value={`$${selected.avgOrderUsd[0].toLocaleString()}–${selected.avgOrderUsd[1].toLocaleString()}`}
                    icon={Banknote}
                  />
                  <StatBox
                    label="Escrow history"
                    value={String(selected.escrowHistory)}
                    icon={ShieldCheck}
                  />
                  <StatBox
                    label="Disputes"
                    value={String(selected.disputes)}
                    icon={AlertTriangle}
                  />
                </section>
                ...
                <section>
                  <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1">
                    <Globe className="h-3 w-3" /> Preferred corridors
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {selected.corridors.map((c) => (
                ...
                <section className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground flex gap-2">
                  ...
                  Buyer's contact details and registration documents are released only after the
                  buyer accepts your quote or trade invitation.
                </section>
                ...
                <section className="grid grid-cols-2 gap-2">
                  ...
                  />
                  <ActionBtn
                    label="Invite to Trade File"
                    icon={UserPlus}
                    onClick={() => toast.success("Trade file invitation sent")}
                  />
                  <ActionBtn
                    label="Request Proof of Funds"
                    icon={Wallet}
                    onClick={() => toast.success("Request sent to buyer")}
                  />
                  <ActionBtn
                    label="Offer Escrow Terms"
                    icon={ShieldCheck}
                    onClick={() => toast.success("Escrow offer queued")}
                  />
                  <ActionBtn
                    label="Message via Canta"
                    icon={MessageSquare}
