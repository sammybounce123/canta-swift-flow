...
const DOCS: Doc[] = [
  {
    id: "DOC-2041",
    name: "Commercial Invoice — SH-9012.pdf",
    type: "Commercial Invoice",
    linkedTo: "Shipment SH-9012",
    uploadedBy: "Aisha B.",
  ...
  {
    id: "DOC-2040",
    name: "Bill of Lading — SH-9012.pdf",
    type: "Bill of Lading",
    linkedTo: "Shipment SH-9012",
    uploadedBy: "Ops Team",
  ...
  {
    id: "DOC-2039",
    name: "Packing List — SP-2026-0131.xlsx",
    type: "Packing List",
    linkedTo: "Supplier Payment SP-2031",
    uploadedBy: "Tunde B.",
  ...
  {
    id: "DOC-2037",
    name: "SONCAP Certificate.pdf",
    type: "SONCAP",
    linkedTo: "Supplier Payment SP-2042",
    uploadedBy: "Tunde B.",
  ...
  },
  {
    ...
    type: "Inspection",
    linkedTo: "Supplier Payment SP-2055",
    uploadedBy: "QC Vendor",
  ...
  {
    id: "DOC-2035",
    name: "Supplier Contract — Yiwu.pdf",
    type: "Contract",
    linkedTo: "Supplier: Yiwu Fashion",
    uploadedBy: "Adaeze O.",
  ...
  {
    id: "DOC-2034",
    name: "Customs Duty Receipt.pdf",
    type: "Receipt",
    linkedTo: "Shipment SH-8990",
    uploadedBy: "Clearing Agent",
      ...
      docs.filter(
        (d) =>
          ...
          (!q ||
            ...
            d.linkedTo.toLowerCase().includes(q.toLowerCase())),
      ),
    [q, type, status, docs],
  );
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Every trade document in one searchable vault — invoices, BLs, packing lists, customs
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Search by file name or linked shipment / trade file..."
              className="pl-9"
            />
          </div>
          ...
          <Select value={status} onValueChange={(v) => setStatus(v as typeof status)}>
            ...
            <SelectContent>
              {(["All", "Verified", "Pending review", "Action required"] as const).map((s) => (
            ...
            <tbody>
              {filtered.map((d) => (
                <tr key={d.id} className="border-t border-border hover:bg-secondary/30">
                  ...
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={`text-[10px] ${tone(d.status)}`}>
                      {d.status}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => toast.success(`Previewing ${d.name}`)}
                    >
...
function UploadDialog({
  ...
  onSubmit: (d: Omit<Doc, "id" | "uploadedAt" | "uploadedBy" | "status">) => void;
  onClose: () => void;
}) {
  ...
  function submit() {
    if (!name.trim()) {
      toast.error("Choose a file or enter a name");
      return;
    }
  ...
  return (
    <DialogContent className="max-w-md">
      ...
      <div className="space-y-3">
        ...
        <div>
          <Label>Link to (shipment / trade file / supplier)</Label>
          <Input
            value={linkedTo}
            onChange={(e) => setLinkedTo(e.target.value)}
            placeholder="Shipment SH-9012"
          />
        </div>
      </div>
      <DialogFooter>
