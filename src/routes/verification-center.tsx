    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <Badge variant="outline" className="gap-1">
            <ShieldCheck className="h-3 w-3" /> Canta Internal · Trust & Safety
          </Badge>
          <h1 className="text-2xl font-semibold tracking-tight mt-2">Verification Center</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Moderate suppliers and buyers in the Canta Trade Network. Approve verifications, monitor
            risk, and suspend bad actors.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard
          ...
          value={`${VERIFICATION_REQUESTS.length}`}
          icon={Activity}
          tone="text-primary"
        />
        <KpiCard
          label="Verified suppliers"
          value={`${SUPPLIERS.filter((s) => s.status !== "Unverified" && s.status !== "Suspended").length}`}
          icon={Factory}
          tone="text-success"
        />
        <KpiCard
          label="Verified buyers"
          value={`${BUYERS.filter((b) => b.status !== "Unverified" && b.status !== "Suspended").length}`}
          icon={UserCheck}
          tone="text-accent"
        />
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="suppliers" className="mt-4">
          <DirectoryTable
            rows={SUPPLIERS.map((s) => ({
              ...
              status: s.status,
              meta: `${s.completedTx} tx · ${s.disputes} disputes`,
            }))}
          />
        </TabsContent>
        ...
        <TabsContent value="buyers" className="mt-4">
          <DirectoryTable
            rows={BUYERS.map((b) => ({
              id: b.id,
              name: b.name,
              country: `${b.city}, ${b.country}`,
              status: b.status,
              meta: `Pay ${b.paymentScore} · ${b.disputes} disputes`,
            }))}
          />
        </TabsContent>
        ...
        <TabsContent value="disputes" className="mt-4 space-y-3">
          <DisputeCard
            id="DSP-301"
            parties="Lagos Global Imports ⇄ Dubai Auto Parts FZE"
            reason="Late shipment, partial refund requested"
            amount="$18,400"
            status="Open"
          />
          <DisputeCard
            id="DSP-302"
            parties="Nairobi Textiles ⇄ Bursa Steel & Iron"
            reason="Quality mismatch on sample"
            amount="$6,250"
            status="Under review"
          />
        </TabsContent>
        ...
        <TabsContent value="risk" className="mt-4">
          <Card className="p-4">
            <div className="text-sm font-semibold mb-3 flex items-center gap-2">
              <FileWarning className="h-4 w-4 text-warning" /> Risk monitoring queue
            </div>
            ...
            <RiskRow
              name="Yiwu Smart Goods Co."
              reason="New entity, low transaction history"
              risk="Low"
            />
          </Card>
        </TabsContent>
...
function RequestCard({
  ...
  req: (typeof VERIFICATION_REQUESTS)[number];
  checks: string[];
}) {
    ...
    <Card className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-xs text-muted-foreground">
            {req.country} · submitted {req.submittedAt} · {req.id}
          </div>
        </div>
      ...
      <div className="mt-3">
        <div className="flex items-center justify-between text-xs mb-1">
          <span className="text-muted-foreground">
            {req.checksPassed}/{req.checksTotal} checks passed
          </span>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        {checks.slice(req.checksPassed).map((c) => (
          <Badge key={c} variant="outline" className="text-[10px] gap-1 opacity-60">
      ...
      <div className="mt-4 flex gap-2 flex-wrap">
        <Button size="sm" onClick={() => toast.success(`${req.name} approved`)}>
          <BadgeCheck className="h-4 w-4 mr-2" />
          Approve
        </Button>
        <Button size="sm" variant="outline" onClick={() => toast("Requested more info")}>
          Request more info
        </Button>
        <Button size="sm" variant="outline" onClick={() => toast.error("Profile suspended")}>
          Suspend
        </Button>
      </div>
    </Card>
      ...
      <div className="divide-y divide-border">
        {rows.map((r) => (
          <div key={r.id} className="p-3 flex items-center gap-3 hover:bg-secondary/40">
            ...
            <Button size="sm" variant="ghost" onClick={() => toast(`Opened ${r.id}`)}>
...
function RiskRow({
  ...
  const tone =
    risk === "High" ? "text-destructive" : risk === "Medium" ? "text-warning" : "text-success";
  return (
