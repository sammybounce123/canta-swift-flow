...
const SEED: Payer[] = [
  {
    id: "P-001",
    name: "Adaeze Okafor",
    email: "adaeze@lagos.edu.ng",
    phone: "+234 803 111 2233",
    reference: "STU-2024-001",
    invoice: "INV-9001",
    amount: 1_200,
    ccy: "USD",
    status: "Paid",
  ...
  },
  {
    id: "P-002",
    name: "Tunde Adeyemi",
    email: "tunde@parents.ng",
    phone: "+234 805 222 3344",
    reference: "STU-2024-002",
    invoice: "INV-9002",
    amount: 1_500,
    ccy: "USD",
    status: "Paid",
  ...
  },
  {
    id: "P-003",
    name: "Sarah Mohammed",
    email: "sarah@donors.org",
    phone: "+234 802 333 4455",
    reference: "DON-2024-018",
    invoice: "INV-9003",
    amount: 5_000,
    ccy: "GBP",
    status: "Pending",
  ...
  },
  {
    id: "P-004",
    name: "Ibrahim Yusuf",
    email: "ibrahim@yusuf.com",
    phone: "+234 807 444 5566",
    reference: "STU-2024-003",
    invoice: "INV-9004",
    amount: 900,
    ccy: "USD",
    status: "Paid",
  ...
  },
  {
    ...
    phone: "+1 415 666 7788",
    reference: "DON-2024-019",
    invoice: "INV-9006",
    amount: 2_500,
    ccy: "USD",
    status: "Paid",
...
function PayersPage() {
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Payer details are stored for reconciliation and audit."
      />
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Payers</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Manage students, parents, donors and customers paying your organization.
          </p>
        </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          ...
          <TabsTrigger value="matched">
            Matched ({payers.filter((p) => p.match === "Matched").length})
          </TabsTrigger>
          <TabsTrigger value="unmatched">
            Unmatched ({payers.filter((p) => p.match === "Unmatched").length})
          </TabsTrigger>
          <TabsTrigger value="exceptions">
            Exceptions ({payers.filter((p) => p.match === "Exception").length})
          </TabsTrigger>
        </TabsList>
...
function PayersTable({
  ...
  payers: Payer[];
  onMatch: (id: string, m: MatchStatus) => void;
}) {
  return (
            ...
            {payers.map((p) => (
              <tr key={p.id} className="border-t border-border hover:bg-secondary/30">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full border ${STATUS_TONES[p.status]}`}
                  >
                    {p.status}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full border ${MATCH_TONES[p.match]}`}
                  >
                    {p.match}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  {p.match !== "Matched" ? (
                    <Button size="sm" variant="outline" onClick={() => onMatch(p.id, "Matched")}>
                      Match
                    </Button>
                  ) : (
                    <Button size="sm" variant="ghost" onClick={() => toast.success("Receipt sent")}>
                      Receipt
                    </Button>
                  )}
                </td>
...
function AddPayerDialog({
  ...
  const submit = () => {
    if (!d.name || !d.reference) {
      toast.error("Name and reference are required");
      return;
    }
    onAdd({
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Invoice">
            <Input value={d.invoice} onChange={(e) => setD({ ...d, invoice: e.target.value })} />
          </Field>
          <Field label="Amount">
            <Input
              type="number"
      ...
      <Card className="p-5 shadow-card">
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm font-semibold flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-success" /> KYB
          </div>
          <span className={`text-[10px] px-2 py-0.5 rounded-full border ${KYB_TONES[kyb]}`}>
            {kyb}
          </span>
        </div>
        ...
        <div className="mt-3 space-y-2">
          {docs.map((d, i) => (
            <div key={d.name} className="flex items-center justify-between text-xs">
              ...
              <Button size="sm" variant="ghost" className="h-6 px-2" onClick={() => upload(i)}>
                <Upload className="h-3 w-3 mr-1" /> {d.uploaded ? "Replace" : "Upload"}
              </Button>
            </div>
          ))}
        </div>
