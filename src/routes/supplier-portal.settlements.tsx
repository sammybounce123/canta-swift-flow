...
export const Route = createFileRoute("/supplier-portal/settlements")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "RMB paid and pending, your settlement bank account, expected dates and receipts." },
    ],
  }),
...
function stepIndex(inv: SimpleInvoice) {
  switch (inv.status) {
    case "NGN Received": return 1;
    case "Compliance Review": return 2;
    case "Auto-Converting": return 3;
    case "RMB Settlement Pending": return 4;
    case "RMB Paid": return 6;
    default: return 0;
  }
}
...
function SettlementsPage() {
  ...
  const rows = invoices.filter((i) =>
    ["NGN Received", "Compliance Review", "Auto-Converting", "RMB Settlement Pending", "RMB Paid"].includes(i.status),
  );
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat label="Settlement bank account" value={dest ? `${dest.bankName.split("—")[0].trim()} ${maskAccount(dest.accountNumber)}` : "Not set"} />
        <Stat label="Expected settlement" value={s.rmbPending > 0 ? "1–2 business days after compliance" : "—"} />
      </div>
      ...
      {!dest && (
        <Card className="border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <Link to="/supplier-portal/rmb-bank-account" className="underline">Add RMB bank account</Link>
        </Card>
      )}
      ...
      <Card className="p-4">
        ...
        <div className="space-y-4">
          ...
          {rows.length === 0 && <div className="text-xs text-muted-foreground">No settlements in progress yet.</div>}
        </div>
      </Card>

      <Card className="p-3 text-[11px] italic text-muted-foreground">
        Settlement is never marked RMB Paid without payout provider confirmation, and is only ever paid to a verified
        bank account owned by your business.
      </Card>
    </div>
  );
}
