...
import { Building2, Edit3, Upload, Wallet, ShieldCheck, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            View, edit and manage your merchant identity, payout account and KYB status.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => setCcyOpen(true)}>
            Update settlement currency
          </Button>
          <Button variant="outline" onClick={() => setPayoutOpen(true)}>
            <Wallet className="h-4 w-4 mr-1.5" /> Manage payout account
          </Button>
          ...
          <Button asChild>
            <Link to="/merchant/kyb">
              <ShieldCheck className="h-4 w-4 mr-1.5" /> View KYB
            </Link>
          </Button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
        ...
        <Card className="p-5 shadow-card space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <CheckCircle2 className="h-3 w-3 mr-1" /> {m.kybStatus}
            </Badge>
          </div>
          <div className="pt-2 border-t">
            <div className="text-xs text-muted-foreground">Default payout</div>
            <div className="text-sm font-medium">
              {m.payoutAccount.bank} · {m.payoutAccount.account} ({m.payoutAccount.ccy})
            </div>
          </div>
          <Button asChild variant="outline" size="sm" className="w-full">
            <Link to="/merchant/kyb">Manage KYB</Link>
          </Button>
        </Card>
      </div>
        ...
        {m.documents.length === 0 ? (
          ...
          <div className="space-y-2">
            {m.documents.map((d) => (
              <div
                key={d.name}
                className="flex items-center justify-between text-sm border-b border-border/40 py-2"
              >
                <div>
      ...
      <CcyDialog
        ...
        current={m.settlementCurrency}
        onSave={(v) => {
          save({ ...m, settlementCurrency: v });
          setCcyOpen(false);
          toast.success(`Settlement currency set to ${v}`);
        }}
      />
      <PayoutDialog
        open={payoutOpen}
        onClose={() => setPayoutOpen(false)}
        current={m.payoutAccount}
        onSave={(p) => {
          save({ ...m, payoutAccount: p });
          setPayoutOpen(false);
      ...
      <UploadDialog
        ...
        onUpload={(name) => {
          save({
            ...m,
            documents: [
              { name, uploadedAt: new Date().toISOString().slice(0, 10) },
              ...m.documents,
            ],
          });
          setDocOpen(false);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              value={f.merchantType}
              onChange={(e) => setF({ ...f, merchantType: e.target.value })}
            />
          </div>
          <div>
            <Label>Country</Label>
          ...
          <div className="sm:col-span-2">
            <Label>Collection use cases (comma separated)</Label>
            <Input
              value={f.useCases.join(", ")}
              onChange={(e) =>
                setF({
                  ...f,
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Settlement currency</DialogTitle>
        </DialogHeader>
...
function PayoutDialog({
  ...
  open: boolean;
  onClose: () => void;
  current: Merchant["payoutAccount"];
  onSave: (p: Merchant["payoutAccount"]) => void;
}) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <Input value={f.ccy} onChange={(e) => setF({ ...f, ccy: e.target.value })} />
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>
