  ...
  head: () => ({
    meta: [
      { title: "Shipments — Canta Importer" },
      {
        name: "description",
        content:
          "Upload your Bill of Lading, add container details and follow your shipment to delivery.",
      },
...
function ShipmentsPage() {
  ...
  return (
    <div className="space-y-6">
      <ReadinessBar
        status="Demo Preview"
        cue="Shipment updates are recorded here. Tracking data in this demo is illustrative."
      />
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <Ship className="h-5 w-5 text-primary" /> Shipments
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Upload your BL, add container details, and track your goods to delivery.
          </p>
        </div>
        <BLDialog />
      </header>
        ...
        {s.shipments.map((sh) => (
          <Card key={sh.id} className="p-4 shadow-card">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {sh.shippingLine}
                  {sh.vessel ? ` · ${sh.vessel}` : ""} · {sh.container}
                </div>
              </div>
            ...
            <div className="mt-3 text-xs text-muted-foreground space-y-0.5">
              <div>
                {sh.portLoading} → {sh.portDestination}
              </div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div>
                ...
                <Select
                  value={sh.status}
                  onValueChange={(v) => {
                    updateShipment(sh.id, { status: v as ImporterShipment["status"] });
                    notify("Shipment", `${sh.blNumber} update: ${v}.`);
                  }}
                >
              ...
              <div>
                ...
                <Input
                  className="h-9"
                  defaultValue={sh.container}
                  onBlur={(e) => updateShipment(sh.id, { container: e.target.value })}
                />
              </div>
            </div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={() => {
                  addDocument({
                    name: `Bill of Lading — ${sh.blNumber}.pdf`,
                    type: "Bill of Lading",
                    linkedShipment: sh.id,
                  });
              ...
              <Button
                ...
                variant="ghost"
                onClick={() => {
                  notify("Shipment", `Update requested for ${sh.blNumber}.`);
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => sendToBot(`${sh.blNumber} / ${sh.container}`)}
              >
              ...
              <label className="flex items-center gap-2 text-xs text-muted-foreground ml-auto">
                ...
                <Switch
                  checked={sh.notify}
                  onCheckedChange={(v) => updateShipment(sh.id, { notify: v })}
                />
              </label>
            </div>
          </Card>
...
function BLDialog() {
  ...
  const save = () => {
    ...
    addDocument({
      name: `Bill of Lading — ${f.blNumber}.pdf`,
      type: "Bill of Lading",
      linkedShipment: id,
      linkedPayment: f.paymentRef || undefined,
    });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2">
            ...
            <Select value={f.paymentRef} onValueChange={set("paymentRef")}>
              <SelectTrigger>
                <SelectValue placeholder="Link to a supplier payment (optional)" />
              </SelectTrigger>
              <SelectContent>
                {s.payments.map((p) => (
          ...
          <div>
            ...
            <Input value={f.blNumber} onChange={(e) => set("blNumber")(e.target.value)} />
          </div>
          <div>
            <Label>Shipping line</Label>
            <Input value={f.shippingLine} onChange={(e) => set("shippingLine")(e.target.value)} />
          </div>
          <div>
            <Label>Vessel name (optional)</Label>
            <Input value={f.vessel} onChange={(e) => set("vessel")(e.target.value)} />
          </div>
          <div>
            <Label>Container number</Label>
            <Input value={f.container} onChange={(e) => set("container")(e.target.value)} />
          </div>
          <div>
            <Label>Port of loading</Label>
            <Input value={f.portLoading} onChange={(e) => set("portLoading")(e.target.value)} />
          </div>
          <div>
            <Label>Port of destination</Label>
            <Input
              value={f.portDestination}
              onChange={(e) => set("portDestination")(e.target.value)}
            />
          </div>
          <div className="sm:col-span-2">
        ...
        <DialogFooter>
          <Button onClick={save}>Upload BL</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function sendToBot(reference: string) {
  ...
  if (!CANTA_WHATSAPP_BOT_NUMBER) {
    ...
    toast.info("WhatsApp bot number not configured in demo", {
      description: "Tracking message copied to your clipboard.",
    });
    return;
  }
  window.open(
    `https://wa.me/${CANTA_WHATSAPP_BOT_NUMBER}?text=${encodeURIComponent(msg)}`,
    "_blank",
    "noopener",
  );
}
    ...
    <Card className="p-4 sm:p-5 shadow-card">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="font-semibold flex items-center gap-2">
            <MessageCircle className="h-4 w-4 text-primary" /> Track shipment on WhatsApp
          </h2>
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Send your BL or container number to Canta&apos;s WhatsApp bot to receive shipment
            updates.
          </p>
        </div>
        <label className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <Switch
            ...
            onCheckedChange={(v) => {
              setAlerts(v);
              toast.success(v ? "WhatsApp notifications on" : "WhatsApp notifications off");
            }}
          />
        </label>
      </div>
      ...
      <div className="mt-3 flex flex-col sm:flex-row gap-2">
        <Input
          ...
          placeholder="BL number or container number"
        ...
        <Button
          variant="outline"
          onClick={() => {
            void navigator.clipboard?.writeText(botMessage(ref || "BL / container number"));
            toast.success("Tracking message copied");
          }}
        >
