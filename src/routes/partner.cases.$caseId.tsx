    ...
    return (
      <div className="text-center py-20">
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link to="/partner/cases">Back to cases</Link>
        </Button>
      </div>
    );
  }
          ...
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <Badge variant="outline" className="text-[10px]">
                <Building2 className="h-3 w-3 mr-1" /> {PARTNER_ORG.name}
              </Badge>
            ...
            <div className="text-sm text-muted-foreground mt-1 flex flex-wrap items-center gap-x-4 gap-y-1">
              <span className="flex items-center gap-1">
                <Building2 className="h-3.5 w-3.5" /> {c.property}
              </span>
              <span className="flex items-center gap-1">
                <MapPin className="h-3.5 w-3.5" /> {c.propertyLocation}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="p-5 shadow-card lg:col-span-2">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
              ...
              <Row
                label="Referral marketer"
                value={getMarketer(c.assignedMarketerId)?.name ?? "—"}
              />
              <Row
                label="Created by"
                value={getMarketer(c.createdBy ?? c.assignedMarketerId)?.name ?? "—"}
              />
          ...
          <Card className="p-5 shadow-card">
            ...
            <div className="space-y-2">
              <Button className="w-full justify-start" onClick={() => setTab("FX Quote")}>
                <ArrowLeftRight className="h-4 w-4 mr-2" /> Generate FX quote
              </Button>
              <Button
                variant="outline"
                className="w-full justify-start"
                onClick={() => setTab("Payment Link")}
              ...
              >
                <Upload className="h-4 w-4 mr-2" /> Upload KYC documents
              </Button>
              <Button
                variant="outline"
                className="w-full justify-start"
                onClick={() => {
                  inviteToCanta(c.id, actor);
                  toast.success("Client invited to activate Canta");
                }}
              >
        ...
        <Card className="p-6 shadow-card">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={() => {
                  markPaidToSolicitor(c.id, actor);
                  toast.success("Marked paid to solicitor");
                }}
              >
          ...
          {canSeeSolicitorBankDetails(role) ? (
            ...
            <div className="text-xs text-muted-foreground italic border-t pt-3">
              Solicitor bank details are restricted. Ask a Partner Admin or Finance Viewer for
              access.
            </div>
          )}
        </Card>
      ...
      {tab === "Activity" && (
        <Card className="p-6 shadow-card">
          <div className="text-sm font-semibold mb-3 flex items-center gap-2">
            <ClipboardList className="h-4 w-4 text-primary" /> Activity log
          </div>
...
function DocumentsTab({ caseId, docs, actor }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    <div className="space-y-5">
      <Card className="p-5 shadow-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <Select value={type} onValueChange={(v) => setType(v as any)}> // eslint-disable-line @typescript-eslint/no-explicit-any
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          />
          <Button onClick={add}>
            <Upload className="h-4 w-4 mr-1.5" /> Add document
          </Button>
        </div>
      </Card>
      ...
      <Card className="p-5 shadow-card">
        ...
        {docs.length === 0 ? (
          ...
          <ul className="text-sm divide-y border rounded-lg">
            {docs.map((d: any) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
...
function FxQuoteTab({
  ...
  const onGenerate = () => {
    if (!gbp) {
      toast.error("Enter GBP amount");
      return;
    }
    if (!rate) {
      toast.error("Enter FX rate");
      return;
    }
      ...
      <Card className="p-6 shadow-card space-y-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            Enter the GBP amount and the FX rate. The NGN total and Canta fee are calculated
            automatically.
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t text-sm">
          ...
          <Row
            label="NGN client pays"
            value={ngnPreview ? `₦${ngnPreview.toLocaleString()}` : "—"}
            highlight
          />
        </div>
      </Card>
          ...
          <div className="flex items-center justify-between">
            ...
            {expired ? (
              ...
              <Badge variant="outline" className="text-warning border-warning/30">
                <Clock className="h-3 w-3 mr-1" /> {Math.floor(remaining / 60000)}m{" "}
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            ...
            <Button
              ...
              onClick={() => {
                ...
                window.open(
                  `mailto:${clientEmail ?? ""}?subject=${encodeURIComponent("Your FX quote from Kingsbridge Property Partners × Canta")}&body=${body}`,
                  "_blank",
                );
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                ...
                const msg = encodeURIComponent(
                  `Your FX quote ${quote.reference}: ₦${quote.ngnTotal.toLocaleString()} at 1 GBP = ₦${quote.rate.toLocaleString()}. Pay here: ${full}`,
                );
            ...
            >
              ...
              {paymentLink ? "Preview payment page" : "Create payment link"}
            </Button>
          </div>
        </Card>
      )}
...
function PaymentLinkTab({ c, actor }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
          ...
          <Button
            onClick={() => {
              ...
              if (newLink) {
                ...
                if (typeof window !== "undefined")
                  window.open(newLink.url, "_blank", "noopener,noreferrer");
              } else {
      ...
      {link && (
        <Card className="p-6 shadow-card space-y-3">
          ...
          <div className="text-[11px] text-muted-foreground border-t pt-2">
            ...
            {link.sentAt ? ` · sent ${new Date(link.sentAt).toLocaleString()}` : ""}
            {link.openedAt ? ` · opened ${new Date(link.openedAt).toLocaleString()}` : ""}
          </div>
        </Card>
      )}
    </div>
...
function VerificationTab({ c }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  const v = c.verification;
  return (
    <Card className="p-6 shadow-card">
      <div className="text-sm font-semibold mb-3 flex items-center gap-2">
        <ShieldCheck className="h-4 w-4 text-primary" /> Client verification
      </div>
      {!v ? (
        <div className="text-sm text-muted-foreground">
          Client has not yet completed verification. Once they open the payment link they'll enter
          their BVN, confirm consent and proceed to funding.
        </div>
      ) : (
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          ...
          <Row
            label="Submitted at"
            value={v.submittedAt ? new Date(v.submittedAt).toLocaleString() : "—"}
          />
...
function FundingTab({ c, actor }: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  const f = c.funding;
  ...
  const record = () => {
    ...
    if (!n) {
      toast.error("Enter NGN amount received");
      return;
    }
    ...
    <div className="space-y-5">
      <Card className="p-6 shadow-card space-y-3">
        <div className="text-sm font-semibold flex items-center gap-2">
          <Banknote className="h-4 w-4 text-primary" /> Funding
        </div>
        {f ? (
          <dl className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-3 text-sm">
            ...
            <Row
              label="Received NGN"
              value={f.receivedNGN ? `₦${f.receivedNGN.toLocaleString()}` : "—"}
            />
            <Row label="Payer name" value={f.payerName ?? "—"} />
            <Row label="Reference" value={f.reference ?? "—"} />
            <Row
              label="Received at"
              value={f.receivedAt ? new Date(f.receivedAt).toLocaleString() : "—"}
            />
      ...
      <Card className="p-6 shadow-card space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <Input
            ...
            placeholder="NGN amount received"
            inputMode="numeric"
          />
        ...
        <div className="flex gap-2">
          <Button onClick={record}>
            <CheckCircle2 className="h-4 w-4 mr-1.5" /> Record funding
          </Button>
          <Button
            variant="outline"
            onClick={() => {
              convertFx(c.id, actor);
              toast.success("FX converted, payout processing");
            }}
          >
...
function Row({
  ...
  return (
    <div>
      ...
      <dd
        className={`flex items-center gap-1.5 mt-0.5 ${highlight ? "text-base font-semibold" : "text-sm font-medium"}`}
      >
