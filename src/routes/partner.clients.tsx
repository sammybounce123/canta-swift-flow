...
import {
  visibleCases, statusTone, formatGBP, getMarketer, getSolicitor, CASE_STATUSES,
  type CaseStatus,
} from "@/lib/partner";
...
function PartnerClientsPage() {
  ...
  const filtered = useMemo(() => rows.filter((r) =>
    ...
    (!q || [r.client, r.marketer, r.property, r.solicitor].some((v) => v.toLowerCase().includes(q.toLowerCase())))
  ), [rows, q, status]);
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3 sm:flex sm:flex-wrap sm:justify-between">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button asChild><Link to="/partner/new-referral"><Plus className="h-4 w-4 mr-1.5" /> New referral</Link></Button>
        </div>
      </div>
      ...
      <Card className="p-4 shadow-card">
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2">
          <div className="relative">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search client, marketer, property, solicitor..." className="pl-9" />
          </div>
          <Select value={status} onValueChange={(v) => setStatus(v as CaseStatus | "all")}>
            <SelectTrigger className="w-full sm:w-56"><SelectValue placeholder="All case statuses" /></SelectTrigger>
            <SelectContent>
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.caseId} className="border-t hover:bg-secondary/30">
                  ...
                  <td className="py-3 px-3"><Badge variant="outline" className={`text-[10px] ${statusTone(r.caseStatus)}`}>{r.caseStatus}</Badge></td>
                  ...
                  <td className="py-3 px-3 text-xs text-muted-foreground tabular-nums">{r.lastActivity}</td>
                  <td className="py-3 px-3 text-right">
                    <Button asChild size="sm" variant="ghost">
                      <Link to="/partner/cases/$caseId" params={{ caseId: r.caseId }}>Open</Link>
                    </Button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={10} className="text-center text-sm text-muted-foreground py-10">No clients match your filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
