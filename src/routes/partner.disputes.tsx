...
type DisputeStatus = "Open" | "Under Review" | "Waiting on Client" | "Waiting on Canta" | "Resolved" | "Closed";
type Dispute = {
...
function PartnerDisputesPage() {
  ...
  function save(next: Dispute[]) { setList(next); try { localStorage.setItem(KEY, JSON.stringify(next)); } catch { /* noop */ } }

  const stats = useMemo(() => ({
    open: list.filter((d) => d.status === "Open" || d.status === "Under Review").length,
    waiting: list.filter((d) => d.status === "Waiting on Client" || d.status === "Waiting on Canta").length,
    ...
    <div className="space-y-6">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">Open, track and resolve disputes tied to your client payment cases.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-1.5" /> Open dispute</Button></DialogTrigger>
            ...
            <tbody>
              {list.map((d) => (
                <tr key={d.id} className="border-t hover:bg-secondary/30">
                  ...
                  <td className="py-3 px-3 text-right tabular-nums">£{d.amount.toLocaleString()}</td>
                  <td className="py-3 px-3 text-xs">{d.issueType}</td>
                  <td className="py-3 px-3"><Badge variant="outline" className={`text-[10px] ${STATUS_TONE[d.status]}`}>{d.status}</Badge></td>
                  <td className="py-3 px-3 text-xs text-muted-foreground tabular-nums">{d.lastUpdate}</td>
                  <td className="py-3 px-3 text-right">
                    <Button size="sm" variant="ghost" onClick={() => setActive(d)}>Open</Button>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <DialogContent className="max-w-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2"><Label>Client</Label><Input value={f.client} onChange={(e) => setF({ ...f, client: e.target.value })} /></div>
        <div><Label>Marketer</Label><Input value={f.marketer} onChange={(e) => setF({ ...f, marketer: e.target.value })} /></div>
        <div><Label>Case ref</Label><Input value={f.caseRef} onChange={(e) => setF({ ...f, caseRef: e.target.value })} placeholder="BC-CASE-..." /></div>
        <div><Label>Solicitor</Label><Input value={f.solicitor} onChange={(e) => setF({ ...f, solicitor: e.target.value })} /></div>
        <div><Label>Amount (GBP)</Label><Input type="number" value={f.amount} onChange={(e) => setF({ ...f, amount: Number(e.target.value) || 0 })} /></div>
        <div className="sm:col-span-2"><Label>Issue type</Label>
          <Select value={f.issueType} onValueChange={(v) => setF({ ...f, issueType: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              {["Funding mismatch", "Name mismatch", "Document issue", "Solicitor beneficiary issue", "Expired quote payment", "Other"].map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      </div>
      <DialogFooter>
        ...
        <Button onClick={() => { if (!f.client.trim()) { toast.error("Client is required"); return; } onSubmit(f); }}>Open dispute</Button>
      </DialogFooter>
    </DialogContent>
  );
}

function DisputeDetail({ dispute, onClose, onUpdate }: { dispute: Dispute; onClose: () => void; onUpdate: (d: Dispute) => void }) {
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-48 overflow-y-auto text-xs space-y-1 border rounded p-2 bg-secondary/30">
            {dispute.notes.map((n, i) => (
              <div key={i} className="flex items-start gap-2"><FileText className="h-3 w-3 mt-0.5 text-muted-foreground shrink-0" />{n}</div>
            ))}
          </div>
        </div>
