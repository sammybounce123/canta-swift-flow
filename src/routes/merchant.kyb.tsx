    ...
    <div className="space-y-6">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage your verification documents. KYB stays current even after approval.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            onClick={() => toast.success("Verification summary downloaded")}
          >
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {status === "Approved" && (
                <span className="text-xs text-muted-foreground">
                  You can still update documents or download your summary.
                </span>
              )}
            </div>
          </div>
      ...
      <Card className="p-5 shadow-card">
        ...
        <div className="space-y-2">
          {docs.map((d) => (
            <div
              key={d.key}
              className="flex items-center justify-between gap-3 border-b border-border/40 py-2 flex-wrap"
            >
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {d.fileName ? `${d.fileName} · uploaded ${d.uploadedAt}` : "Not uploaded yet"}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className={docTone(d.status)}>
                  {d.status}
                </Badge>
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Request verification review</DialogTitle>
          </DialogHeader>
