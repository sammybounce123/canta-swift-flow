...
export const Route = createFileRoute("/merchant/kyb")({
  head: () => ({ meta: [{ title: "Merchant KYB — Canta" }] }),
  component: MerchantKybPage,
});

type KybStatus = "Not Started" | "In Progress" | "Submitted" | "Approved" | "Rejected" | "More Info Required" | "Expired";
...
const SEED: Doc[] = [
  { key: "registration", label: "Company registration",          required: true, status: "Approved", uploadedAt: "2026-03-02", fileName: "CertOfInc.pdf" },
  { key: "director-id",  label: "Director / Owner ID",            required: true, status: "Approved", uploadedAt: "2026-03-02", fileName: "DirectorID.pdf" },
  { key: "address",      label: "Proof of business address",      required: true, status: "Approved", uploadedAt: "2026-03-02", fileName: "AddressProof.pdf" },
  { key: "tax",          label: "Tax document (optional)",        required: false, status: "Uploaded", uploadedAt: "2026-04-10", fileName: "TIN.pdf" },
  { key: "payout",       label: "Payout account proof",           required: true, status: "Approved", uploadedAt: "2026-03-02", fileName: "BankLetter.pdf" },
];

const KEY = "canta:merchant:kyb:v1";
...
function MerchantKybPage() {
  ...
  const [comments] = useState<string[]>([
    "2026-03-05 — Compliance: documents reviewed, KYB approved.",
    "2026-04-10 — Optional tax document added.",
  ]);

  useEffect(() => { setDocs(loadDocs()); }, []);
  function save(next: Doc[]) { setDocs(next); try { localStorage.setItem(KEY, JSON.stringify(next)); } catch { /* noop */ } }
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-primary shrink-0" /> Merchant KYB / Verification
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Manage your verification documents. KYB stays current even after approval.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={() => setReviewOpen(true)}><RefreshCw className="h-4 w-4 mr-1.5" /> Request review</Button>
        </div>
      </header>
