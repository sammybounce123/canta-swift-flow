      ...
      data
        ...
        .filter((c) => {
          ...
          if (status === "successful")
            return ["Paid to Solicitor", "Receipt Uploaded"].includes(c.status);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {role === "marketer"
              ? "Payouts on your referred clients."
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((c) => {
                ...
                const receipted =
                  c.status === "Receipt Uploaded" || c.status === "Paid to Solicitor";
                return (
              ...
              {rows.length === 0 && (
                <tr>
                  <td
                    colSpan={showMarketerFilter ? 12 : 11}
...
function PayoutDetailDialog({
  ...
  payout: ReturnType<typeof visibleCases>[number];
  onClose: () => void;
}) {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Payout · {payout.clientName}</DialogTitle>
        </DialogHeader>
        <div className="text-xs text-muted-foreground">
          {payout.property} · {sol?.firm} · {formatGBP(payout.amountGBP)}
        </div>
        <div className="flex items-center gap-2">
          <UiBadge variant="outline" className={`text-[10px] ${statusTone(payout.status)}`}>
            {payout.status}
          </UiBadge>
          <span className="text-[11px] text-muted-foreground">
            Ref: {payout.paymentReference ?? `BC/${payout.id}/COMPL`}
          </span>
        </div>
            ...
            return (
              <div key={stage} className="flex gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < STAGES.length - 1 && (
                    <div
                      className={`w-px flex-1 min-h-[24px] ${idx < completedCount ? "bg-success/40" : "bg-border"}`}
                    />
                  )}
                </div>
                <div className="pb-4">
                  <div
                    className={`text-sm font-medium ${state === "pending" ? "text-muted-foreground" : ""}`}
                  >
                    {stage}
                  </div>
