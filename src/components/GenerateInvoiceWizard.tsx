...
function emptyItem(): InvoiceItem {
  return {
    id: crypto.randomUUID?.() ?? `it_${Math.random()}`,
    description: "",
    quantity: 1,
    unit: "pcs",
    unitPrice: 0,
...
export function GenerateInvoiceWizard({
  ...
  const estReceivable =
    selectedQuote?.settlementCurrency === "USD" ? +(total / 7.2).toFixed(2) : total;
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a draft invoice for a Nigerian buyer. Review all details before creating the
              payment request.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex items-center gap-2 text-xs">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className={`flex-1 h-1.5 rounded ${step >= n ? "bg-primary" : "bg-muted"}`}
              />
            ))}
          </div>
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground">
            Step {step} of 3 ·{" "}
            {step === 1 ? "Buyer & FX quote" : step === 2 ? "Invoice details" : "Review & confirm"}
          </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Select value={buyerCompany} onValueChange={setBuyerCompany}>
                  ...
                  <SelectContent>
                    {BUYERS.map((b) => (
                      <SelectItem key={b.company} value={b.company}>
                        {b.company}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <Card className="p-3 text-xs space-y-1 bg-secondary/30">
                  <div>
                    <span className="text-muted-foreground">Contact:</span> {buyer.name}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Email:</span> {buyer.email}
                  </div>
                  <div>
                    <span className="text-muted-foreground">WhatsApp:</span> {buyer.phone}
                  </div>
                </Card>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={quoteId} onValueChange={setQuoteId}>
                  <SelectTrigger>
                    <SelectValue placeholder="Choose FX quote" />
                  </SelectTrigger>
                  <SelectContent>
                    {quotes.map((q) => (
                      <SelectItem key={q.id} value={q.id} disabled={isQuoteExpired(q)}>
                        {q.id} · {q.invoiceCurrency} → {q.settlementCurrency} · Rate {q.rate}{" "}
                        {isQuoteExpired(q) ? "· Expired" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {selectedQuote && (
                  <Card className="p-3 text-xs grid grid-cols-2 gap-2 bg-secondary/30">
                    <div>
                      <span className="text-muted-foreground">Quote:</span> {selectedQuote.id}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Currency:</span>{" "}
                      {selectedQuote.invoiceCurrency}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Rate:</span> {selectedQuote.rate} NGN/
                      {selectedQuote.invoiceCurrency}
                    </div>
                    <div className="inline-flex items-center gap-1">
                      <Lock className="h-3 w-3" /> Expires:{" "}
                      {formatCountdown(selectedQuote.expiresAt)}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Payout currency:</span>{" "}
                      {selectedQuote.settlementCurrency}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Payout account:</span>{" "}
                      {PAYOUT_ACCOUNT.label}{" "}
                      <Badge className="ml-1 bg-emerald-100 text-emerald-800">Verified</Badge>
                    </div>
                  </Card>
                )}
                {expired && (
                  <div className="text-xs text-destructive bg-destructive/10 border border-destructive/30 rounded p-2 flex items-center justify-between">
                    <span>
                      This FX quote has expired. Generate a new quote before creating the invoice.
                    </span>
                    <Button size="sm" variant="outline" onClick={refreshQuote}>
                      <RefreshCw className="h-3.5 w-3.5 mr-1" /> Refresh FX Quote
                    </Button>
                  </div>
                )}
              </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Shipment / container ref (optional)">
                  <Input
                    value={shipmentRef}
                ...
                <Field label="Buyer reference (optional)">
                  <Input
                ...
                <Field label="Supplier company">
                  <Input value={SUPPLIER.company} disabled />
                </Field>
                <Field label="Supplier address">
                  <Input value={SUPPLIER.address} disabled />
                </Field>
                <Field label="Buyer company">
                  <Input value={buyer.company} disabled />
                </Field>
                        ...
                        <tr key={it.id} className="border-t">
                          ...
                          <td className="p-1">
                            <Input
                              ...
                              onChange={(e) =>
                                setItems((v) =>
                                  v.map((x, i) => (i === idx ? { ...x, unit: e.target.value } : x)),
                                )
                              }
                            />
                          </td>
          ...
          {step === 3 && (
            <div className="space-y-3">
              <Card className="p-4 space-y-2 text-sm">
                ...
                <Row
                  k="FX quote"
                  v={selectedQuote ? `${selectedQuote.id} @ ${selectedQuote.rate}` : "—"}
                />
                <Row k="Buyer will pay (NGN)" v={`₦${ngnBuyerPays.toLocaleString()}`} />
                <Row k="Canta fee" v={`₦${cantaFee.toLocaleString()}`} />
                <Row
                  k="Est. supplier receivable"
                  v={`${estReceivable.toLocaleString()} ${selectedQuote?.settlementCurrency ?? currency}`}
                />
                <Row
                  k="Quote expires"
                  v={selectedQuote ? formatCountdown(selectedQuote.expiresAt) : "—"}
                />
              ...
              <div className="text-xs bg-amber-50 text-amber-800 border border-amber-200 rounded p-2">
                The buyer&apos;s NGN payment details will not be generated at this stage. They will
                be generated only after the buyer opens the payment request, completes verification,
                accepts the current quote, and confirms payment.
              </div>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={confirmed} onCheckedChange={(v) => setConfirmed(!!v)} />I confirm
                that the invoice and buyer details are correct.
              </label>
            </div>
          )}
          ...
          <DialogFooter className="gap-2 flex-wrap">
            ...
            {step === 1 && (
              <Button disabled={!selectedQuote || expired} onClick={() => setStep(2)}>
      ...
      <Dialog open={successOpen} onOpenChange={setSuccessOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Invoice {createdInvoice?.invoiceNumber} has been created and linked to{" "}
              {createdInvoice?.buyer.company}.
            </DialogDescription>
          </DialogHeader>
          {createdInvoice && (
            <Card className="p-3 text-sm space-y-1.5">
              ...
              <Row
                k="Amount"
                v={`${createdInvoice.total.toLocaleString()} ${createdInvoice.currency}`}
              />
              <Row
                k="Supplier receivable"
                v={`${createdInvoice.estReceivable?.toLocaleString() ?? "—"} ${createdInvoice.settlementCurrency ?? ""}`}
              />
              <Row
                k="Quote expires"
                v={
                  createdInvoice.quoteExpiresAt
                    ? formatCountdown(createdInvoice.quoteExpiresAt)
                    : "—"
                }
              />
          ...
          <DialogFooter className="gap-2 flex-wrap">
            ...
            <Button
              ...
              onClick={() => {
                ...
                toast.info(`Opening invoice ${createdInvoice?.invoiceNumber}`);
              }}
            >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Review the invoice and current FX quote before generating the secure buyer payment link.
          </DialogDescription>
        </DialogHeader>
        <Card className="p-3 text-sm space-y-1.5">
          ...
          <Row
            k="Supplier receivable"
            v={`${(invoice.estReceivable ?? 0).toLocaleString()} ${invoice.settlementCurrency ?? ""}`}
          />
        </Card>
        {sent && (
          <div className="space-y-2">
            <div className="text-xs bg-secondary/40 border rounded p-2 font-mono break-all">
              {link}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              >
                <MessageCircle className="h-3.5 w-3.5 mr-1" /> Share on WhatsApp
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => toast.success("Payment request downloaded")}
              >
                <Download className="h-3.5 w-3.5 mr-1" /> Download Payment Request
              </Button>
              <Button size="sm" onClick={() => toast.success(`Sent to ${invoice.buyer.email}`)}>
                <Send className="h-3.5 w-3.5 mr-1" /> Send to Buyer
              </Button>
            </div>
            <div className="text-[11px] text-muted-foreground italic">
              Buyer NGN payment details generate only after the buyer opens the link, passes
              KYC/KYB, accepts the valid quote, and confirms the payment request.
            </div>
          </div>
        )}
        <DialogFooter>
          {!sent ? (
            <>
              <Button
                variant="outline"
                onClick={() => {
                  invoiceStore.update(invoice.id, { paymentRequestStatus: "Pending" });
                  onOpenChange(false);
