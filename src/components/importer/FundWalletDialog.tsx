...
export function FundWalletDialog({
  ...
  const timelineIndex = !record
    ? 0
    : record.status === "Wallet credited"
      ? 5
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Funding rails available today are NGN bank transfer and USDT. Your wallet is credited
            ...
            {(["NGN", "USDT"] as const).map((m) => (
              ...
              >
                <div className="font-medium">
                  {m === "NGN" ? "NGN — Nigerian bank transfer" : "USDT — stablecoin transfer"}
                </div>
                <div className="text-xs text-muted-foreground">
                  {m === "NGN"
                    ? "Transfer to your Canta collection account."
                    : "Send USDT on TRC20, ERC20 or BEP20."}
                </div>
              </button>
            ))}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                placeholder={method === "NGN" ? "5,000,000" : "2,000"}
              />
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Supplier payment top-up"
              />
            </div>
            ...
            {method === "USDT" && (
              <p className="text-xs rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 flex gap-2">
                ...
                Send only USDT on the selected network. Sending another token or using the wrong
                network may delay or lose funds.
              </p>
            )}
            ...
            <div className="rounded-lg border border-border p-3 space-y-2 text-sm">
              {method === "NGN" ? (
                <>
                  ...
                  <Detail
                    k="Account number"
                    v={NGN_COLLECTION_ACCOUNT.accountNumber}
                    onCopy={copy}
                  />
                ...
                <>
                  ...
                  <Detail
                    k="Wallet address"
                    v={USDT_ADDRESSES[record.network ?? network]}
                    onCopy={copy}
                  />
                  <Detail
                    k="Minimum confirmations"
                    v={String(USDT_CONFIRMATIONS[record.network ?? network])}
                  />
                </>
              )}
              <Detail
                k={method === "NGN" ? "Payment reference" : "Deposit reference"}
                v={record.reference}
                onCopy={copy}
              />
              ...
              <Detail
                k="Expires"
                v={record.expiresAt ? new Date(record.expiresAt).toLocaleString() : "—"}
              />
            </div>
              ...
              {!record.status.includes("confirmation submitted") ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (rc) {
                      toast.success(`${method} wallet credited`, {
        ...
        {step === 3 && receipt && (
          <div className="space-y-3">
            <Badge className="text-[10px]">
              <Check className="h-3 w-3" /> Wallet credited
            </Badge>
            <div className="rounded-lg border border-border p-3 space-y-2 text-sm">
  ...
  return (
    <div className="rounded-lg border border-border p-3 space-y-2">
      ...
      <p className="text-[11px] text-muted-foreground">
        Wallets are funded with NGN or USDT only, but you can pay suppliers anywhere — quote against
        any popular currency, whether or not you hold that wallet.
      </p>
      ...
      {amount > 0 ? (
        !quote ? (
            ...
            <div className="rounded-md bg-muted/40 px-3 py-2 space-y-1.5">
              ...
              <div className="flex justify-between items-center border-t border-border pt-1.5">
                <span className="text-xs text-muted-foreground">
                  Supplier receives (indicative)
                </span>
                <span className="text-sm font-semibold">{fmtAnyCcy(liveReceive, target)}</span>
              </div>
            </div>
            <p className="text-[10px] text-muted-foreground">
              This is a live indicative rate. Lock it to guarantee the rate for {QUOTE_LOCK_SECONDS}{" "}
              seconds.
            </p>
          ...
          <>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium">
                1 {method} = {quote.rate >= 1 ? quote.rate.toFixed(2) : quote.rate.toFixed(6)}{" "}
                {quote.target}
              </span>
            </div>
            <div className="flex justify-between items-center rounded-md border border-border px-2.5 py-2">
              ...
              <span className="text-sm font-semibold">
                {fmtAnyCcy(quote.receive, quote.target)}
              </span>
            </div>
            ...
            {expired ? (
              <div className="flex items-center justify-between gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-2.5 py-2">
                <span className="text-[11px] text-destructive flex items-center gap-1.5">
                  <AlertTriangle className="h-3.5 w-3.5" /> Quote expired — refresh to see the
                  current rate.
                </span>
              ...
              <div className="flex items-center justify-between gap-2 rounded-md bg-muted/40 px-2.5 py-2">
                <span className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                  <Timer className="h-3.5 w-3.5" /> Rate locked — valid for{" "}
                  <b className="tabular-nums">{mmss}</b>
                </span>
                <Button size="sm" variant="ghost" onClick={lock}>
                  <RefreshCw className="h-3.5 w-3.5" /> Re-lock
                </Button>
              </div>
            )}
          </>
        ...
        <p className="text-[11px] text-muted-foreground">
          Enter an amount to lock a {target} quote.
        </p>
      )}
    </div>
  );
