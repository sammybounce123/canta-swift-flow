...
export function PaySupplierWizard() {
  ...
  const pickSaved = (id: string) => {
    ...
    setF((x) => ({
      ...
      country: sup.country,
      contact: sup.contact ?? "",
      channel: sup.contactChannel ?? "",
      bankName: sup.bankName,
      accountName: sup.accountName,
  ...
  const submit = () => {
    if (!f.bankName || !f.accountNumber) {
      toast.error("Supplier bank details are required");
      return;
    }
    if (!f.invoice) {
      toast.error("Upload the supplier invoice before submitting");
      return;
    }
    if (!quoteAcceptedAt || expired) {
      toast.error("Accept a valid FX quote first");
      return;
    }
    if (!enough) {
      toast.error("Insufficient balance", {
        description: `Fund your ${f.fundingWallet} wallet before submitting.`,
      });
      return;
    }
    ...
    const id = addPayment({
      ...
      fundedFrom: `${f.fundingWallet} Wallet`,
      documents: docs,
    });
    docs.forEach((d) =>
      addDocument({
        name: d,
        type: d === f.invoice ? "Commercial invoice" : "Supporting document",
        linkedPayment: id,
      }),
    );
    ...
    toast.success(`Supplier Payment ${id} submitted`, {
      description: "Receipt is generated after payout confirmation.",
    });
  ...
  const canNext =
    step === 0
      ? Boolean(f.name && f.country && f.bankName && f.accountNumber)
      : step === 1
        ? amount > 0
        : step === 2
          ? Boolean(f.invoice)
          : step === 3
            ? Boolean(quoteAcceptedAt) && !expired
            : step === 4
              ? enough
              : true;
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <ReadinessBar
        status="Demo Preview"
        cue="Illustrative FX rates. Payments are reviewed before payout."
      />
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your supplier does not need a Canta account. Canta pays directly to the supplier's bank
          account after payment, FX, and compliance review.
        </p>
      </header>
      ...
      {step === 0 && s.drafts.length > 0 && (
        <Card className="p-3 shadow-card space-y-2">
          ...
          {s.drafts.map((d) => (
            <div
              key={d.id}
              className="rounded-lg border border-border p-2 flex flex-wrap items-center justify-between gap-2 text-sm"
            >
              <span>
                {d.supplier || "Unnamed supplier"} · {d.amountLabel} · saved {d.at}
              </span>
      ...
      <Card className="p-4 sm:p-5 shadow-card space-y-4">
        {step === 0 && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="sm:col-span-2">
                <Label>WhatsApp / email (optional)</Label>
                <Input value={f.channel} onChange={(e) => set("channel")(e.target.value)} />
              </div>
              <div>
                <Label>Supplier bank name *</Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                value={f.amount}
                onChange={(e) => {
                  setF((x) => ({ ...x, amount: e.target.value }));
                  setQuoteAcceptedAt(null);
                  setQuoteIssuedAt(Date.now());
                }}
                placeholder="180,000"
            ...
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                placeholder="Apparel order #2291"
              />
            </div>
            <div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Button
                ...
                onClick={() => set("invoice")(`Commercial invoice — ${f.name || "supplier"}.pdf`)}
              >
            ...
            <div>
              ...
              <Button
                ...
                className="w-full justify-start"
                onClick={() =>
                  set("supporting")(`Supporting document — ${f.name || "supplier"}.pdf`)
                }
              >
            ...
            <p className="text-xs text-muted-foreground">
              A supplier invoice is required before a payment can be submitted for review.
            </p>
          </div>
        )}
            ...
            {expired ? (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  Expired FX quotes cannot be used.
                </span>
                <Button
                  size="sm"
                  variant="outline"
              ...
              <Badge className="text-[10px]">
                <Check className="h-3 w-3" /> Quote accepted
              </Badge>
            ) : (
        ...
        {step === 4 && (
          <div className="space-y-3">
              ...
              return (
                ...
                >
                  <div className="font-medium flex items-center gap-1.5">
                    <Wallet className="h-4 w-4" /> {ccy} Wallet
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Available {fmtWallet(w?.available ?? 0, ccy)} · this payment needs{" "}
                    {fmtWallet(need, ccy)}
                  </div>
                </button>
              );
            ...
            {enough ? (
              <p className="text-xs text-muted-foreground">
                Use wallet balance — {fmtWallet(cost, f.fundingWallet)} will be reserved when you
                submit.
              </p>
            ) : (
              <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 space-y-2">
                <div className="text-sm font-semibold flex items-center gap-1.5">
                  <AlertTriangle className="h-4 w-4 text-destructive" /> Insufficient balance
                </div>
                <p className="text-xs text-muted-foreground">
                  You need to fund your wallet before submitting this supplier payment.
                </p>
                <div className="flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    onClick={() => {
                      setF((x) => ({ ...x, fundingWallet: "NGN" }));
                      setFundOpen(true);
                    }}
                  >
                    Fund NGN Wallet
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => navigate({ to: "/importer/balance" })}
                  >
                    Go to Balance
                  </Button>
                  <Button
                    ...
                    onClick={() => {
                      saveDraft({
                        supplier: f.name,
                        amountLabel: `${f.currency} ${amount.toLocaleString()}`,
                        form: f as unknown as Record<string, unknown>,
                      });
                      toast.success("Payment saved as draft", {
                        description: "Fund your wallet, then resume the draft.",
                      });
        ...
        {step === 5 && (
          <div className="rounded-lg border border-border p-4 space-y-2 text-sm">
            ...
            <Row
              k="Supplier bank details"
              v={`${f.bankName} · ${f.accountNumber}${f.swift ? ` · ${f.swift}` : ""}`}
            />
            ...
            <Row
              k="Documents"
              v={[f.invoice, f.supporting].filter(Boolean).join(", ") || "None uploaded"}
            />
            <Row
              k="Funding source"
              v={`${f.fundingWallet} Wallet — ${fmtWallet(cost, f.fundingWallet)}`}
            />
            <Row k="Estimated timeline" v="1–3 business days after compliance review" />
            <p className="text-xs text-muted-foreground pt-2">
              Payments are reviewed before payout. Receipts are generated after provider
              confirmation.
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap justify-between gap-2 pt-2">
          <Button variant="ghost" disabled={step === 0} onClick={() => setStep((x) => x - 1)}>
            Back
          </Button>
          {step < STEPS.length - 1 ? (
            <Button disabled={!canNext} onClick={() => setStep((x) => x + 1)}>
              Continue
            </Button>
          ) : (
...
function AddSupplierDialog({
  ...
  onSaved: (id: string, d: NewSupplier) => void;
}) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2">
            <Label>WhatsApp / email (optional)</Label>
            <Input value={d.channel} onChange={(e) => set("channel")(e.target.value)} />
          </div>
          <div>
            <Label>Supplier bank name *</Label>
            <Input value={d.bankName} onChange={(e) => set("bankName")(e.target.value)} />
          </div>
          <div>
            <Label>Supplier account name</Label>
            <Input value={d.accountName} onChange={(e) => set("accountName")(e.target.value)} />
          </div>
          <div>
            <Label>Account number / IBAN *</Label>
            <Input value={d.accountNumber} onChange={(e) => set("accountNumber")(e.target.value)} />
          </div>
          <div>
            <Label>SWIFT / BIC</Label>
            <Input value={d.swift} onChange={(e) => set("swift")(e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <Label>Bank address (optional)</Label>
            <Input value={d.bankAddress} onChange={(e) => set("bankAddress")(e.target.value)} />
          </div>
          <div>
            <Label>Settlement currency</Label>
