  ...
  return {
    ...
    fundingReceipts: [
      ...
      },
      {
        ...
        ccy: "USDT",
        amount: 3_200,
        method: "USDT transfer (TRC20)",
        providerRef: "PRV-USDT-771903",
        at: iso(-5),
        status: "Wallet credited",
      },
    ],
    ...
    suppliers: [
      {
        ...
        country: "China",
        contact: "Mei Lin",
        contactChannel: "mei@yiwufashion.cn",
        bankName: "Bank of China",
        accountName: "Yiwu Fashion Co. Ltd",
        accountNumber: "6217 0021 8899 4410",
        swift: "BKCHCNBJ",
        currency: "RMB",
      ...
      },
      {
        ...
        country: "Turkey",
        contact: "Ahmet K.",
        contactChannel: "ahmet@istex.tr",
        bankName: "Ziraat Bankasi",
        accountName: "Istanbul Textiles A.S.",
        accountNumber: "TR33 0006 1005 1978 6457",
        swift: "TCZBTR2A",
        currency: "EUR",
        status: "Saved",
        lastPayment: iso(-40),
      ...
      {
        id: "SUP-003",
        name: "Northwind Trading FZE",
        country: "United Arab Emirates",
        contact: "Sara N.",
        contactChannel: "sara@northwind.ae",
        bankName: "Emirates NBD",
        accountName: "Northwind Trading FZE",
        accountNumber: "AE07 0331 2345 6789 0123",
        swift: "EBILAEAD",
        currency: "USD",
        status: "Saved",
      },
    ...
    payments: [
      ...
      {
        id: "SP-2026-0141",
        supplier: "Northwind Trading FZE",
        country: "United Arab Emirates",
        bank: "Emirates NBD",
        accountNumber: "AE07 0331 2345 6789 0123",
        swift: "EBILAEAD",
    ...
    documents: [
      {
        id: "DOC-2001",
        name: "Commercial invoice — Yiwu 2291.pdf",
        type: "Commercial invoice",
        linkedPayment: "SP-2026-0139",
        status: "Approved",
        uploadedAt: iso(-12),
      },
      {
        id: "DOC-2002",
        name: "Packing list — Yiwu 2291.pdf",
        type: "Packing list",
        linkedPayment: "SP-2026-0139",
        status: "Approved",
        uploadedAt: iso(-12),
      ...
      {
        id: "DOC-2003",
        name: "Proforma invoice — Istex.pdf",
        type: "Proforma invoice",
        linkedPayment: "SP-2026-0140",
        status: "Under review",
        uploadedAt: iso(-3),
      },
      {
        id: "DOC-2004",
        name: "Bill of Lading — MSCU4410.pdf",
        type: "Bill of Lading",
        linkedShipment: "SH-3301",
        status: "Uploaded",
        uploadedAt: iso(-6),
      },
    ],
    shipments: [
      {
        id: "SH-3301",
        blNumber: "MSCUNG2291",
        paymentRef: "SP-2026-0139",
        shippingLine: "MSC",
        vessel: "MSC Aurora",
        container: "MSCU4410221",
        portLoading: "Ningbo",
        portDestination: "Apapa, Lagos",
        eta: iso(9),
        status: "In transit",
        notify: true,
      },
      {
        id: "SH-3302",
        blNumber: "TRKX88120",
        paymentRef: "SP-2026-0140",
        shippingLine: "Arkas Line",
        vessel: "Arkas Star",
        container: "ARKU7781002",
        portLoading: "Izmir",
        portDestination: "Onne, Port Harcourt",
        eta: iso(3),
      ...
      },
      {
        ...
        blNumber: "ENBD40021",
        shippingLine: "Maersk",
        container: "MRKU5512009",
        portLoading: "Jebel Ali",
        portDestination: "Apapa, Lagos",
        eta: iso(-4),
        status: "Clearing",
        notify: false,
    ...
    notifications: [
      {
        id: "N-1",
        kind: "Shipment",
        text: "SH-3302 is arriving soon at Onne, Port Harcourt.",
        at: iso(0),
        read: false,
      },
      {
        id: "N-2",
        kind: "Compliance",
        text: "SP-2026-0140 is under compliance review before payout.",
        at: iso(-1),
        read: false,
      },
      {
        id: "N-3",
        kind: "Receipt",
        text: "Receipt RC-2026-0139 is ready to download.",
        at: iso(-11),
        read: true,
      },
    ],
    ...
    prefs: {
      ...
      saveSupplierByDefault: true,
      usdtWarnings: true,
      emailFundingReceipts: true,
      whatsappPaymentNotifications: true,
    },
    business: {
      name: "Bakare Imports Ltd",
      contact: "Tunde Bakare",
      email: "tunde@bakareimports.ng",
...
export function notify(kind: ImporterNotification["kind"], text: string) {
  update((s) => ({
    ...s,
    notifications: [
      { id: `N-${Date.now()}`, kind, text, at: new Date().toISOString().slice(0, 10), read: false },
      ...s.notifications,
    ].slice(0, 60),
  }));
}
...
export function addPayment(p: Omit<SupplierPayment, "id" | "createdAt">) {
  ...
  update((s) => ({
    ...s,
    payments: [{ ...p, id, createdAt: new Date().toISOString().slice(0, 10) }, ...s.payments],
  }));
  ...
  update((s) =>
    s.wallets.some((w) => w.ccy === ccy)
      ...
      : {
          ...
          wallets: [
            ...s.wallets,
            { ccy, available: 0, pending: 0, status: "Active", lastActivity: today() },
          ],
          walletTx: [
            {
              id: `WT-${Date.now()}`,
...
function applyWallet(
  s: ImporterState,
  ccy: WalletCcy,
  patch: (w: ImporterWallet) => ImporterWallet,
): ImporterState {
  const wallets = s.wallets.map((w) =>
    w.ccy === ccy ? { ...patch(w), lastActivity: today() } : w,
  );
  ...
  return {
    ...s,
    wallets,
    ngnBalance: ngn?.available ?? s.ngnBalance,
    usdtBalance: usdt?.available ?? s.usdtBalance,
  };
}
...
export function debitWallet(ccy: WalletCcy, amount: number) {
  update((s) =>
    applyWallet(s, ccy, (w) => ({ ...w, available: Math.max(0, w.available - amount) })),
  );
}
...
export function addWalletTx(tx: Omit<WalletTx, "id" | "at"> & { at?: string }) {
  update((s) => ({
    ...s,
    walletTx: [
      { id: `WT-${Date.now()}-${Math.floor(Math.random() * 1000)}`, at: tx.at ?? today(), ...tx },
      ...s.walletTx,
    ],
  }));
}
...
export function startFunding(input: {
  ...
  update((s) =>
    applyWallet({ ...s, funding: [entry, ...s.funding] }, input.method, (w) => ({
      ...w,
      pending: w.pending + input.amount,
    })),
  );
  addWalletTx({
    ccy: input.method,
...
export function simulateProviderConfirmation(id: string) {
  ...
  update((s) => {
    ...
    return {
      ...withWallet,
      funding: withWallet.funding.map((x) =>
        x.id === id ? { ...x, status: "Wallet credited", providerRef, receiptNo } : x,
      ),
      fundingReceipts: [
        {
          receiptNo,
          ...
          amount: f.amount,
          method:
            f.method === "NGN" ? "NGN bank transfer" : `USDT transfer (${f.network ?? "TRC20"})`,
          providerRef,
          at: today(),
      ...
      walletTx: withWallet.walletTx.map((t) =>
        t.reference === id && t.type === "Wallet funding"
          ? { ...t, status: "Completed" as const, receiptNo }
          : t,
      ),
    };
  });
  notify(
    "Balance",
    `${f.method} wallet credited — funding ${id} confirmed. Receipt ${receiptNo} is available.`,
  );
  return receiptNo;
}
...
export function addDocument(
  d: Omit<ImporterDoc, "id" | "uploadedAt" | "status"> & { status?: ImporterDoc["status"] },
) {
...
export function addShipment(
  sh: Omit<ImporterShipment, "id" | "status" | "notify"> & { notify?: boolean },
) {
  const id = `SH-${Math.floor(3300 + Math.random() * 600)}`;
  update((s) => ({
    ...s,
    shipments: [
      { id, status: "Documents uploaded", notify: sh.notify ?? true, ...sh },
      ...s.shipments,
    ],
  }));
...
export function updateShipment(id: string, patch: Partial<ImporterShipment>) {
  update((s) => ({
    ...s,
    shipments: s.shipments.map((x) => (x.id === id ? { ...x, ...patch } : x)),
  }));
}
...
export function setPref<K extends keyof ImporterState["prefs"]>(
  k: K,
  v: ImporterState["prefs"][K],
) {
...
export const fmtCcy = (n: number, ccy: string) => `${ccy} ${Math.round(n).toLocaleString("en-US")}`;
 ...
 */
export function remittanceQuote(
  method: "NGN" | "USDT",
  amount: number,
): {
  fee: number;
...
export function buildLockedQuote(
  method: "NGN" | "USDT",
  amount: number,
  target: string,
): LockedQuote {
